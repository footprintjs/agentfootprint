...
Liskov gives us **boundaries that don't leak**. [Palantir-style connected data](doc:connected-data-palantir) gives us **connections within those boundaries**. Together: clean modules + connected data = a runtime that's both fast (Palantir multiplier) and reasonable (Liskov locality).
