...
|---|---|---|
| **THE WHY** | [Connected data — the Palantir lineage](doc:connected-data-palantir) | Palantir's 2003 thesis (connect data, not analysts) returns at agent runtime. Disconnected state burns iterations and tokens. agentfootprint connects four classes of agent data (state · decisions · execution · memory). |
| **THE HOW** | [Modularity — the Liskov lineage](doc:modularity-liskov) | Liskov's ADT + LSP work, applied to flowcharts. Subflows are CLU clusters. CacheStrategy / LLMProvider / ToolProvider are LSP-substitutable. Locality of reasoning is enforced as a runtime invariant. |
| **THE SPINE** | [Strategy everywhere — the port/adapter lineage](doc:strategy-everywhere) | The cache layer's strategy-registry shape (one DSL, N vendor strategies, swap by config) generalized into the library's scaling spine for every observability / cost / status / lens vendor integration — Strategy / Bridge / Ports & Adapters / Provider model / algebraic effects, one pattern in five traditions. |
...
If you just want to use the library, start at [Quick Start](doc:quick-start) instead. The inspiration pages are not required reading.
