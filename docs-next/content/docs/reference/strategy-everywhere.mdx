...
2. **N strategies** — auto-registered by side-effect import from the one grouped `agentfootprint/cache` subpath, each strategy carrying a peer-dep on its vendor SDK. (This memo originally sketched a subpath *per vendor* — `cache-anthropic`, `cache-openai`. What shipped is one subpath per **group**, with one strategy per vendor inside it. The observability adapters below landed the same way, under `agentfootprint/observe`; treat "one grouped subpath, N strategies" as the rule.)
...
Library already ships `memory-agentcore` (`@aws-sdk/client-bedrock-agent-runtime` peer dep). The AWS pattern is established. The AWS-side observability adapters below **shipped in v3.1** — all under the single grouped `agentfootprint/observe` subpath (one factory per vendor, never a new subpath):
...
```typescript
import { composeObservability, consoleObservability, otelObservability } from 'agentfootprint/observe';

const telemetry = composeObservability([
  // every adapter takes ONE options object — `serviceName` is required
  otelObservability({ serviceName: 'my-agent-prod', tracer }), // your OTel collector (Datadog via OTLP, etc.)
  consoleObservability(),                                      // local dev visibility
]);

const stop = agent.enable.observability({ strategy: telemetry });

// The framework does not do this for you:
process.on('SIGTERM', async () => { await telemetry.flush(); telemetry.stop(); stop(); });
```

> **Import paths:** the vendor strategies (`agentcoreObservability`, `cloudwatchObservability`, `xrayObservability`, `otelObservability`), the default sinks (`consoleObservability`, `inMemorySinkCost`, `chatBubbleLiveStatus`, `noopLens`) and the per-group combinators (`composeObservability`, `composeCost`, `composeLiveStatus`, `composeLens`) are all exported from **`agentfootprint/observe`** — the canonical door, and the one to use. The vendor strategies are *also* reachable at `agentfootprint/observability-providers` and the combinators at `agentfootprint/strategies`; both are the older doors kept alive through 8.x. None of them are on the root `agentfootprint` barrel. There is **no single `compose`** — each group has its own typed combinator. Datadog ships no native adapter; point an OTel SDK at Datadog's OTLP endpoint and use `otelObservability`.

`composeObservability(children)` takes **one array argument** — no varargs, no options object — and returns a strategy whose `exportEvent` fans out to every child. Errors are isolated per child; one bad exporter doesn't break others. Same pattern as `withRetry` / `withFallback` in the resilience subsystem. Its `flush()` and `stop()` fan out too, and since 8.12.0 the framework calls them on shutdown: the handle `enable.*` returns carries `flush()` / `stop()`, `agent.shutdown()` drains everything enabled on a runner, and a `standingAgent` flushes when it closes.
...
```typescript
import type { ObservabilityStrategy } from 'agentfootprint/observe';
...
> **Status (4.0.0):** the grouped enablers (`observability`, `cost`, `liveStatus`), the strategy slot + the 4 default sinks (importable from `agentfootprint/observe`), the per-group `compose*` combinators, and the AWS observability adapters (`agentcoreObservability`, `cloudwatchObservability`, `xrayObservability`) plus `otelObservability` have **all shipped** — under the single `agentfootprint/observe` subpath. The deprecated flat methods `enable.thinking` / `enable.logging` were **removed in 4.0.0** (migrate to `enable.liveStatus` / `enable.observability`); `enable.flowchart` is **kept** (it's a real, non-deprecated composition-graph feature). `cost`/`lens` vendor subpaths and `enable.lens` remain planned. The version numbers below are the original plan, kept for the historical record — **and so are the per-vendor subpath names in them**. None of `observability-agentcore`, `observability-cloudwatch`, `observability-xray` or `observability-otel` was ever published as an import path; they are plan labels for what became factories inside `agentfootprint/observe`. Do not copy them into an import.
...
| **v2.8.1** | **AWS Bedrock AgentCore Observability** — planned as `observability-agentcore`; shipped as `agentcoreObservability()` in `agentfootprint/observe`. Pipes events into AgentCore's native observability. Same peer (`@aws-sdk/client-bedrock-agent-runtime`) as `memory-agentcore`, so consumers already integrated with AgentCore Memory get observability for free. | No |
| **v2.8.2** | **AWS CloudWatch Logs** — planned as `observability-cloudwatch`; shipped as `cloudwatchObservability()` in `agentfootprint/observe`. Generic AWS structured logging. Peer: `@aws-sdk/client-cloudwatch-logs`. | No |
| **v2.8.3** | **AWS X-Ray** — planned as `observability-xray`; shipped as `xrayObservability()` in `agentfootprint/observe`. Distributed tracing. Peer: `@aws-sdk/client-xray`. | No |
