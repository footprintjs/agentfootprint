...
The breaker is per-instance, in-process (no Redis dependency). See [the resilience guide](/agentfootprint/guides/resilience/) for composing it with retry + fallback.
...
- [Reliability guide](/agentfootprint/guides/reliability/) — rules-based retry / fail-fast inside the agent loop
- [Pause / resume](/agentfootprint/guides/pausable/) — JSON-checkpointed human-in-the-loop
