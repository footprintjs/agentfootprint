...
- `assumed` — **nobody said why.** The cursor is inside the map and no clause explains how it got there. The weakest rung, and the honest answer where the old code wrote down the strongest one.
...
1. **Engagement provenance** — *why is this map participating at all?* `foundedBy`, `foundedAt` and `foundedOn`. Written once and **never** rewritten. "At iteration 3 the user explicitly activated audit" stays true forever, whatever happens afterwards.
2. **Position evidence** — *why is the cursor on this member?* `at`, `atBy` and `atSince`. The kernel observes position; it never sets it.
3. **Contribution eligibility** — *why is this member's contribution being served right now?* `by`. Recomputed every pass.
