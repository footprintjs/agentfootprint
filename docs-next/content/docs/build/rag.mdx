...
Whichever rule ran, it names itself: `strategy` on the record and on `agentfootprint.memory.retrieved` carries the `RetrievalStrategy.name` that produced the verdicts. Two rules with the same `k` leave otherwise identical records, so that name is what tells them apart.
...
| Name | What it is |
|---|---|
| `RetrievalEvidence` | The whole record for one retrieval: `strategy` (which rule ruled), `queryHash`, `k`, `threshold`, `maxChars` / `charsUsed` (when a size budget was set), `embedderId`, `dimensions`, `selectionOrder`, the counts, `candidates`, `candidatesComplete`, `corpusEmpty`, `namespace`. |
