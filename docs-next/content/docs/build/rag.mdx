...
In agentfootprint, RAG is **the same plumbing as `defineMemory({ type: SEMANTIC, strategy: TOP_K })`** — the same engine, the same observability event (`agentfootprint.context.injected` with `source: 'rag'`), the same multi-tenant identity. `defineRAG` is just a friendlier factory with retrieval-tuned defaults (`topK: 3`, `threshold: 0.7`).

## Define a retriever, attach to an agent

`defineRAG({ id, store, embedder, description?, embedderId?, topK?, threshold? })` returns a `MemoryDefinition`. `agent.rag(definition)` attaches it (alias for `.memory(definition)` — same plumbing, clearer intent). It throws at construction time if `store` lacks `search()`. Pair `embedderId` here with the same value passed to `indexDocuments` so a later embedder swap is filtered out of results:
