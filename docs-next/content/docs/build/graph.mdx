...
## Naming your nodes — the `sf-` prefix is reserved

Framework-owned composition segments are named `sf-*` (`sf-llm-call`, `sf-tools`, `sf-cache`, …), and every downstream reader — commentary, step graphs, the OTel bridge — tells library plumbing from your structure by that prefix alone. `graph()` node ids (like `Parallel.branch()` and `Conditional.when()`) therefore refuse the prefix at declaration time. `RESERVED_SUBFLOW_PREFIX` and `isReservedSubflowSegment` (main barrel) export the law itself, so a viewer or pattern miner can import it instead of hardcoding the string.
