...
`ollama()` is a thin convenience over `openai()` — it lives on the `agentfootprint/providers` subpath (vendor-SDK-backed providers are kept off the main barrel for tree-shaking). Options: `host` (defaults to `http://localhost:11434`), `baseURL` (overrides `host`), `apiKey` (Ollama ignores it; defaults to `'ollama'`), `defaultModel` (defaults to `llama3.2`), and `defaultMaxTokens`.
...
- [OpenAI](doc:openai) — `baseURL` pattern works with any OpenAI-compatible endpoint
