...
Stack them via multiple `.memory(...)` calls on the same agent. Each layer's read subflow runs independently and contributes to the system-prompt slot:
...
Multiple memories on the same agent each get their own scope key — `memoryInjection_${id}` — so they layer cleanly. The unique IDs (`recent`, `facts`, `causal` above) become observability event identifiers. Every layer emits `agentfootprint.context.injected` with `payload.source === 'memory'` and `payload.sourceId` set to the layer's id, so you can filter by the specific layer:

```ts
agent.on('agentfootprint.context.injected', (e) => {
  if (e.payload.source === 'memory' && e.payload.sourceId === 'facts') {
    // the `facts` layer fired — inspect e.payload.contentSummary, etc.
  }
});
```
...
- [Memory guide](doc:memory) — single-layer fundamentals + the 4-types × 7-strategies matrix
- [Causal deep-dive](doc:causal-deep-dive) — snapshot shape, projection, replay
