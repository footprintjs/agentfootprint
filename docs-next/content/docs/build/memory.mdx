...
> ⚠️ **Causal *similarity* recall needs a store that implements `search()`.** The cosine-search above is provided **only by `InMemoryStore`** today — both production adapters (`RedisStore`, `AgentCoreStore`) implement every `MemoryStore` method **except** `search()` (vector search is future work). So in production you can **persist** causal snapshots and recall them with a `WINDOW` strategy (last-N snapshots, no search needed), but **cosine-similarity recall across sessions is not production-ready yet**. Until a vector-search adapter ships, scope durable causal memory to recent-window recall, or run similarity recall in-process via `InMemoryStore`. For in-conversation "why?" that doesn't need any store at all, see [`.selfExplain()`](doc:self-explain).
