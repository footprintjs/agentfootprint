...
<Callout type="info" title="How SUMMARIZE spends your money (9.14.0)">
...
- **`model` is required, with no fallback to the agent's model** — the same law [`.compaction()`](doc:window-strategies) has enforced since 8.14.0. `Agent.memory()` additionally refuses a summarizer that is the agent's own provider *instance* at the agent's own model.
- **A broken summarizer degrades to `WINDOW`, loudly** — one warning, one `memory.strategy_applied` event, and recall proceeds verbatim. A summary that comes back no shorter than the span it would replace is refused and latched, so the same question is not bought twice.
