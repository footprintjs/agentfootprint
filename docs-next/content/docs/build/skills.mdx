...
**DEPRECATED-pending-steps (9.16.0).** The field is stored on
`skill.metadata.refreshPolicy` and nothing in the engine has ever read it — no
re-injection happens. It is superseded by a planned steps-as-data feature and
will **not** be wired up in the meantime; it stays accepted so existing
declarations keep compiling (dev mode warns once per process when one is set),
and it will be removed in the next major after steps ship. To re-surface a body
in a long run today, use `surfaceMode: 'both'` — every `read_skill` call
returns the body afresh, recency-first.
