...
### Declaring the graph: the object form is a union
...
from there) and `model-edge-only` (the only way in is a bare `.route(a, b)`, which
compiles to no trigger at all — the model has to ask, and only from `a`).
