...
`SkillMatch` — either a `RegExp` tested against the user's message, or
`{ keywords: [...] }` (case-insensitive, any keyword present, whole-word at word
edges, so `refund` never fires on "refunds"). Each rule takes **exactly one** of
...
edge carry a serializable `SkillMatchData` (`{ kind: 'regex', source, flags }` or
`{ kind: 'keywords', keywords }`), and the check-up gains three codes — one for every rule, two only
