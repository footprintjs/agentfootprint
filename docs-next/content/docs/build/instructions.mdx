...
The model reads *"You are on action 25 of 30; 5 remain."* This is measured, not
decorative: given its remaining budget a model wrote *"I have 5 steps left,
enough to finish this properly"* and landed the task, where before it spiralled
...
A name outside that set is **refused at define time**, with the three listed in
the error. Nothing renders as a literal in front of a model.
...
- **`prompt` or `promptTemplate`, never both.** They are the same decision
  spelled two ways, and the library will not pick one for you.
- **Not `slot: 'messages'`.** That slot delivers each piece once per run and its
  ledger keys by the content itself, so a template that renders differently every
  action would deliver a *new message* every action, all run. The system-prompt
  slot is re-composed per action by design, which is exactly what a template
  needs.
- **Not cacheable.** A cache marker asserts a stable prefix; these bytes change
  every call. `cache: 'always'` on a template is refused. Because a never-cached
  injection truncates the cached prefix at its declaration position, **declare
  templated instructions last**.
...
A rule that must hold on **every** turn — compliance, content policy, output format —
shouldn't depend on a predicate at all. Use `defineSteering`, which is always on:
...
const agent = Agent.create({ provider, model }).steering(compliance).build();
```

The reason is how a predicate fails. **A predicate that throws is fail-open**: the
Instruction is *skipped* for that iteration, not forced on. That is the right default for
a nice-to-have rule and the wrong one for a rule you can never afford to miss. Steering
(or `defineInstruction` with no `activeWhen`, which is always-on too) has no predicate to
throw.

Every skip is on the record. `agentfootprint.context.evaluated` fires each iteration with
the active ids and a `skippedDetails` entry — `reason: 'predicate-threw'` plus the error —
for each Instruction that was skipped:

```ts
agent.on('agentfootprint.context.evaluated', (e) => {
  for (const s of e.payload.skippedDetails) console.warn(`${s.id} skipped: ${s.reason} ${s.error ?? ''}`);
});
```

Wire that listener before you ship — "which rules actually fired on that turn?" is the
first question an auditor asks. Runnable: [`examples/context-engineering/03-steering.ts`](https://github.com/footprintjs/agentfootprint/blob/main/examples/context-engineering/03-steering.ts).

## One predicate, four trigger kinds

You only ever write one predicate shape — `(ctx: InjectionContext) => boolean` — but
each factory produces one of four underlying `InjectionTrigger` kinds:

| You write | Trigger kind | Active when |
|---|---|---|
| `defineInstruction({ activeWhen })` | `rule` | the predicate returns `true` this iteration |
| `defineInstruction({ … })` with no `activeWhen`, or `defineSteering(…)` | `always` | every iteration |
| an `Injection` built with `{ kind: 'on-tool-return', toolName }` | `on-tool-return` | a tool matching `toolName` (a string, or a `RegExp`) just returned |
| `defineSkill(…)` | `llm-activated` | the model called `read_skill` for that skill |

In practice the `on-tool-return` case is usually written as a `rule` predicate that reads
`ctx.lastToolResult`, as in the section above — same effect, one fewer shape to learn.
`agentfootprint.context.evaluated` also reports `triggerKindCounts`, so you can see how
many of each kind were active on a given turn.
