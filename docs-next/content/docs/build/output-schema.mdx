...
Nothing changes. The last answer stands, `runTyped()` throws `OutputSchemaError` exactly as it always did, and `.outputFallback()` still gets its turn on top. `{ retries }` is a chance to fix the answer, never a change to what happens when it cannot be fixed.

The default is `retries: 0`, and an agent that does not ask for retries is byte-identical to one built before the option existed: no branch in the chart, no key in the commit log, no event.
