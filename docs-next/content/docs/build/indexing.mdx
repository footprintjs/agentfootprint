...
Chunks that exceed the ceiling anyway are **recorded**, not silently clipped:
...
| `IndexReport` | `{ discovered, loaded, chunks, embedded, skipped, removed, failed, truncated, embedderFingerprint, splitter, elapsedMs }`. |
