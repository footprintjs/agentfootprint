...
its ingestion. See [ranking mode](doc:memory-and-stores).
...
- [Memory store adapters](doc:memory-and-stores) — `sqliteVectorStore`, `pgVectorStore`, `s3VectorsStore`, and what each store's `search()` ranks
