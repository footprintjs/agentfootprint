...
4. Inject into the system-prompt slot for this turn

This is the engine behind [`defineRAG`](doc:rag). RAG is just SEMANTIC × TOP_K with retrieval-tuned defaults (`topK: 3`, `threshold: 0.7`).
...
`RedisStore` (exported from `agentfootprint/memory-providers`) ships without `search()` — RedisSearch is a separate Redis module with its own API.

`AgentCoreStore` **does** implement `search()` (since 7.15), but it does not satisfy this contract: AgentCore embeds and ranks server-side, so it needs the query as **text** in `options.text` and ignores the vector the strategy computes. A `SEMANTIC × TOP_K` pipeline hands it a vector alone, so the call throws at runtime, by design — better than ranking nothing and answering `[]`. See [AgentCore Memory](doc:agentcore). Use `InMemoryStore` until vector-capable production adapters land.
...
- [RAG guide](doc:rag) — the document-corpus flavor of this primitive
- [Memory guide](doc:memory) — the full type × strategy matrix
- [Memory store adapters](doc:memory-stores) — vector-capable backends roadmap
