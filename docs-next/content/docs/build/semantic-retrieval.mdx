...
`RedisStore` (exported from `agentfootprint/memory`) ships without `search()` — RedisSearch is a separate Redis module with its own API.
...
- [Memory store adapters](doc:memory-and-stores) — vector-capable backends roadmap
