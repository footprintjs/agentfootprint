...
> **Looking for the simple version?** [`.outputSchema(parser, { retries })`](doc:output-schema) needs no rules and no `.reliability()` config: a failed answer becomes one more ordinary turn of the ReAct loop, with its own LLM bracket and its own cost tick. The page you are on is the OTHER mechanism — retries INSIDE one `call-llm` stage, driven by reliability rules, which is cheaper in iterations and, because it shares one bracket and one cost tick, less visible in the recording. Rules that also need provider failover, circuit breaking or fail-fast belong here; a plain "the shape was wrong, ask again" belongs there.
...
// New helpers (agentfootprint/resilience subpath)
...
```ts
import { defaultStuckLoopRule, lastNValidationErrorsMatch } from 'agentfootprint/resilience';
...
3. **Future option** — if 5+ consumers ship the same `strictOutput({...})` shape over the next 6 months, we promote it to the `agentfootprint/resilience` door in a future minor with a known-good API
...
- [Reliability gate](doc:reliability-gate) — the v2.11.5 foundation this builds on
- [Output schema](doc:output-schema) — the `outputSchema` parser primitive
- [Output fallback](doc:resilience) — the v2.10.x 3-tier degradation chain that catches `ReliabilityFailFastError`
- [Sequence governance](doc:sequence-governance) — the v2.12 sibling recipe (same primitive-extension + recipe pattern)
