...
Subpath keeps the main barrel small + lets bundlers tree-shake when AgentCore isn't used. Prefer `agentfootprint/memory` (the canonical location, shared with `RedisStore`); `agentfootprint/memory` remains as a back-compat alias.
...
- [Memory store adapters](doc:memory-stores) — full adapter matrix
