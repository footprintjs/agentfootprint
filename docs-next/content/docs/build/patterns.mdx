---
title: Patterns
description: Reflection, Tree of Thoughts, Self-Consistency, Debate, Map-Reduce and Swarm — each a small factory over the same primitives, so they compose with each other and with your own flows.
---

Every named pattern in the agent literature is a **composition of the 2 primitives + 3 compositions** from the [concepts taxonomy](doc:key-concepts). Not a new Agent class. Not a new runtime. A recipe.
...
All six are **thin factories** that express a named paper as a composition of primitives. Reading the source ([src/patterns/](https://github.com/footprintjs/agentfootprint/blob/main/src/patterns/)) is the fastest way to learn how to build your own.

| Pattern | Built from | Paper | Returns | Source |
|---|---|---|---|---|
| `reflection` | Loop(Sequence(propose, critique)) | Madaan et al. 2023 (Self-Refine) | `Runner` | [Reflection.ts](https://github.com/footprintjs/agentfootprint/blob/main/src/patterns/Reflection.ts) |
| `tot` | Loop(Parallel(LLM × K) + prune) | Yao et al. 2023 (Tree of Thoughts) | `Runner` | [ToT.ts](https://github.com/footprintjs/agentfootprint/blob/main/src/patterns/ToT.ts) |
| `selfConsistency` | Parallel(LLM × N) + majority vote | Wang et al. 2022 (Self-Consistency) | `Runner` | [SelfConsistency.ts](https://github.com/footprintjs/agentfootprint/blob/main/src/patterns/SelfConsistency.ts) |
| `debate` | Sequence(proposer, critic, judge) | Du et al. 2023 (Multiagent Debate) | `Runner` | [Debate.ts](https://github.com/footprintjs/agentfootprint/blob/main/src/patterns/Debate.ts) |
| `mapReduce` | Sequence(split, Parallel(LLM × N) + reduce) | Dean & Ghemawat 2004 | `Runner` | [MapReduce.ts](https://github.com/footprintjs/agentfootprint/blob/main/src/patterns/MapReduce.ts) |
| `swarm` | Loop(Conditional(route-to-agent)) | OpenAI Swarm 2024 | `Runner` | [Swarm.ts](https://github.com/footprintjs/agentfootprint/blob/main/src/patterns/Swarm.ts) |

> **Hierarchy (Swarm)** — also a pattern (an LLM-driven routing decision picks which agent handles the next turn). It ships as the `swarm(...)` factory like the others; the recipe is `Loop(Conditional(route-to-agent))`. See [Concepts → Swarm (hand-off)](doc:swarm). Paper: OpenAI Swarm (2024).
...
If you build a shape worth reusing, the source of the patterns in [src/patterns/](https://github.com/footprintjs/agentfootprint/blob/main/src/patterns/) is ~85–230 lines each — copy one and modify.
