...
1. **Source injection** — the source material (docs, policy text, retrieved chunks, prior decisions) lands in the system-prompt slot via [RAG](doc:rag), [Memory](doc:memory), or `defineFact`. The LLM sees it as supplied context, above the conversation.
...
The [RAG guide](doc:rag) is the standard pattern. `defineRAG` retrieves top-K matching chunks above a strict threshold; if the threshold is missed, NO chunks inject (the LLM gets no context). This is intentional — *garbage low-confidence chunks make hallucination MORE likely, not less*.
...
When chunks DO match, they land in the system-prompt slot — one system message carrying every match as a citation block. Observability surfaces show exactly which chunks were retrieved per turn.

## Causal memory as grounding for follow-ups

For *cross-run* grounding (the customer asks "why did you tell me that earlier?"), [Causal memory](doc:memory) persists snapshots of past runs. New questions cosine-match past queries; matching snapshots inject the stored run. The follow-up answer is grounded in what actually happened, not reconstruction.

This is the differentiator no other framework has. See [Memory guide § Causal memory](doc:memory#causal-memory--replay-decisions-not-just-messages).
...
- [RAG guide](doc:rag) — the strict-threshold retrieval pattern
- [Memory guide](doc:memory) — Causal memory for cross-run grounding
- [Observability guide](doc:observability) — the full 59-event taxonomy
