...
The `discovery_failed` event still fires; the `reliability` rule decides whether to retry / fall back / fail-fast. See [Reliability gate](doc:reliability-gate).
...
- [Connect to an MCP server](doc:mcp-client) — the shipped MCP tool source in full
- [Serve your tools over MCP](doc:mcp-serve) — `mcpServe(tools)`, the other direction
- [Tool providers](doc:tool-providers) — `staticTools` / `gatedTools` / `skillScopedTools` (the sync 90% case)
- [Reliability gate](doc:reliability-gate) — declarative retry / fallback / fail-fast over discovery failures
- [Observability](doc:observability) — the three `tools.discovery_*` events in the full taxonomy
