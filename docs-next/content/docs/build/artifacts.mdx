...
This page is the feature surface: the adapters, the option shapes, the events, the runnable examples. The mechanism behind them — the port's refusals, the three dispatch laws, what a ref is *not* allowed to move, every claim carrying a status, and one worked refusal captured from a real run — is [Artifact architecture](doc:artifacts-architecture).
...
### Declare datasets once, independently of transport

`withDatasetArtifacts` wraps a local, HTTP-backed or MCP tool with a
`DatasetResultAdapter`. The adapter's `describe(result, args)` returns a
`DatasetResultPlan`: explicit datasets plus a projection that turns storage
receipts into the compact result the model receives. Returning `undefined`
leaves a non-dataset result unchanged. The library does not guess which arrays
are tables or invent their field meanings.

Each `DatasetArtifactInput` has a unique key, an artifact declaration and an
optional source artifact. `DatasetPublication` contains the key and a
`DatasetArtifactReceipt` for each declared artifact. A receipt says either
`stored` with its metadata, or `unavailable` with a reason. Source references
join the dataset's existing parent references. `stageDatasetArtifacts` exposes
the same publication step to pipelines that already own tool execution and
projection; the wrapper delegates to it.

```ts
import { withDatasetArtifacts } from 'agentfootprint';

const referencedTool = withDatasetArtifacts(fetchSales, {
  describe(result) {
    return {
      datasets: [{
        key: 'sales',
      ...
      project([publication]) {
        if (publication.artifact.status !== 'stored') {
          return { available: false, reason: publication.artifact.reason };
        }
        return {
          dataset: publication.artifact.meta,
...
**Required for references:** configure `Agent.create({ artifacts: store })`
and supply the producer adapter. A built-in store is sufficient; implementing
a custom store or exposing storage as an LLM tool is optional. The dispatcher
provides the scope-bound `ctx.artifacts`; a follow-up tool can redeem a dataset
through `wants` without refetching its source. Without a store, this wrapper
does not call the adapter and returns the original tool result. A host that
requires reference-only context must enforce its normal result projection or
refusal policy as well.

**A frontend is optional.** A chatbot can complete the whole flow on the
backend. When a UI is present, its configured artifact resolver calls the host,
which checks the same authenticated scope. The reference is neither a storage
URL nor authorization by itself. Existing `httpArtifactResolver` reads an
artifact through the host; it is not a pagination or database query API.
Shared deployments must configure identity verification: session IDs alone
are not authentication.

Writes are independent, not a transaction. A failed source write can leave a
readable dataset with explicitly unavailable provenance; a failed dataset write
never produces a ticket. Tickets are rechecked after the batch to detect
eviction, but may still expire later. The projection decides how to disclose
each unavailable receipt. Declaration shapes are validated before writing;
the store retains responsibility for serialization, budgets and parent proofs.

These helpers accept **materialized results**. They do not turn a 2 GB download
into a bounded-memory query. Streaming storage and server-side query providers
are separate capabilities; an MCP resource URI is not automatically a local
artifact reference.

Runnable chatbot-only example, including a second-turn calculation:
[`examples/artifacts/dataset-result.ts`](https://github.com/footprintjs/agentfootprint/blob/main/examples/artifacts/dataset-result.ts).
The storage design follows the established
...
- **A declared ref the tool's own schema marks `required` is refused by name when the model omits it** (9.38.0) — at both dispatch doors, including the resume door a human approval comes back through, and independently of the agent-wide `toolArgValidation` dial. An *optional* declared argument the model omits is a legitimate choice: the call runs and `ctx.wanted` simply has no entry for it. The full before/after — and why the dial used to decide this — is in [Artifact architecture](doc:artifacts-architecture#a-required-ref-omitted-is-now-refused-by-name-9380).
...
**One trap to know before you pick a number:** the substitute replaces the result string everywhere downstream — history, `lastToolResult`, `toolResults` — which is exactly where skill-graph `when` edges and `rule` triggers look, so turning placement on or moving `maxInlineChars` can change which edge fires for a graph that matches on result *text*. It is the intended layering (routing judges what the model was told), and the way to be immune to it is to key edges on `onToolReturn` or `onToolStatus`. Reasoned through in [Artifact architecture](doc:artifacts-architecture#the-footgun-placement-rewrites-the-string-your-routing-reads).

### `resultKind` — a placed result the wants rail can spend (9.70.0)

Read the two rules above together and a gap falls out. The `wants` kind check
is **exact** — no wildcards, no hierarchy — and placement mints under
`tool-result/<toolName>`. So an agent whose consuming tool declares
`wants: { dataset: 'dataset/rows' }` refuses the very ticket placement just
minted, as a kind mismatch. The framework hands out a ticket its own rail
will not take, and the honest workaround was to re-mint by hand at the seam.

`Tool.resultKind` closes it from the producing end. A tool declares the
artifact kind its placed result is minted under, in its consumers' vocabulary:
...
const chart = defineTool<{ dataset: string }, string>({
  name: 'chart',
  description: 'Chart a stored dataset. Pass the art_… ref from get_rows.',
  inputSchema: { type: 'object', properties: { dataset: { type: 'string' } }, required: ['dataset'] },
  wants: { dataset: 'dataset/rows' }, // ← resolves now
  execute: async (args) => summarize(JSON.parse(args.dataset)),
});
```

- **The matcher is untouched.** Nothing here loosens `wants`; exact match is
  what makes a ticket a promise, and a rail that guessed at families would
  hand a tool the wrong parcel. What changes is the **mint**, which now speaks
  the author's vocabulary instead of the framework's.
- **Declared, never inferred** — the `capabilities` / `resultClass` law. Only
  the tool's author knows what it produces; a kind guessed from a tool name
  would be a promise the library cannot keep.
- **Refused at `defineTool` if it could never be redeemed.** A blank or
  whitespace-only kind throws by name (the exported `assertResultKind` is the
  same check, for consumers assembling `Tool` objects by hand) — a kind is
  what a ticket is matched against, and a blank one matches nothing.
- **Zero-cost when omitted:** the mint is `tool-result/<toolName>`,
  byte for byte what it always was. `placedResultKind(toolName, declared?)`
  is the one place that decision is made, and it is exported so a `wants`
  declaration can compose the default without spelling the prefix.
- **It only ever moves the KIND.** The ref, the payload, the ticket's shape,
  the `artifacts.minted` event and the precedence order are all unchanged —
  the kind simply travels through `meta.kind` into every one of them.
...
- `as` is consumer vocabulary (`'bar-chart'`, `'table'`, `'image'`…) — **stored as data, never validated**. The shipped renderer picks a component from the artifact's `kind` (the producer's word, stamped at mint), not from `as`, so the model is not the one choosing a renderer at all — see [Artifact architecture](doc:artifacts-architecture#the-model-cannot-invent-a-component--true-but-not-for-the-reason-you-would-guess).
...
With a store attached, files a [code run](doc:tools) hands back **in-band** (`CodeResult.artifacts[].data` — an additive field an adapter fills when it can return the bytes) are minted under the run's scope — kind `file/<ext>` from the producer's own filename, mediaType from the adapter's statement, a well-known-extension table, or the payload's shape — and the rendered result names the ref (`[artifact: report.csv, 29 bytes, stored as art_… (file/csv) — route this ref…]`). Since 9.38.0 each such mint also carries `parentRefs` stamped from the input refs this same call resolved (absent, never `[]`, when none did). Entries without `data` stay described-only, exactly as before. **No runner shipped here fills that field yet** — `localCodeRunner` leaves it absent rather than guessing which files in the caller's own working directory were outputs; what a runner owes to produce them is spelled out in [Artifact architecture](doc:artifacts-architecture#outputs-are-a-gap-and-the-gap-is-named). **Since 9.26.0 the leg runs both ways:** `codeRunnerTool({ wants })` stages the resolved payload INTO the session as a file through `CodeSession.stageInputs`, so a dataset reaches the interpreter without one byte of it entering the prompt. See [Staging refs into a code session](/docs/build/tools#staging-refs-into-a-code-session-9260).
...
A finished run already has a shape every viewer reads: `{ snapshot, events,
structure }`, produced by `recordRun`. What nobody shipped was the boring half
in between — *where* a completed run's recording goes so a screen can ask for it
later. Every deployment that wanted the Lens over the wire wrote the same twenty
lines, and each one wrote a slightly different, slightly wrong version of
retention and scoping.

All four of those problems already had an answer here. A recording is simply an
artifact:
...
The frontend redeems it with the operation it already speaks:
`{ op: 'artifact-get', ref }` returns the recording, and
`observeRecording(JSON.parse(text))` draws it. **Zero new wire operations were
needed.** Retention rides the store, so recordings age out under the same ttl
and byte budget everything else does, and a sweep reports itself on
`agentfootprint.artifacts.expired` exactly as any other eviction does.

`recordings: true` takes the default naming; `AgentRecordingsOptions` —
`{ label }` — names them yourself, and the label is used **verbatim**. A static
label repeats on every run on purpose: what distinguishes two recordings is the
ref and `origin.runId`, and a library that decorated your label to make it
unique would be overruling the name you chose.

### What it costs, stated rather than discovered

Recording a run means an event tail and a boundary recorder for its duration,
and the mint is one store write on the way **out** of `run()`. The answer is
fully composed before the write begins and the write can never change it — but
`run()` does return after it rather than before. That is deliberate: a
fire-and-forget write is a recording lost whenever the container exits with the
reply, which is exactly the deployment that wants this most.

A mint that **fails** degrades to the old behaviour: the answer is returned
unchanged and the reason lands on the record as
`agentfootprint.artifacts.refused`. A run never fails because its recording
could not be filed. Nothing is minted for a run that paused (the turn is not
over — the resume mints its own) or threw.

The payload is the recording's **JSON text** — the same bytes
`JSON.stringify(recorder.toRecording())` produces. `recordRun` states that
`snapshot` and `structure` are the runner's *own* objects held by reference, and
an in-process store handed those would keep a live view into a finished run's
state; serializing detaches it, once. A recording JSON cannot carry (a cyclic
snapshot) is refused at the mint by `UnserializableRecordingError` rather than
at whatever tried to read it.

Unset — the default — no recorder is attached, no events are captured, nothing
is minted, and the run is byte-identical to every earlier release.

For custom harnesses the pure half is public: `recordingPutInput(recording,
facts)` turns a recording into the `PutArtifactInput` that stores it, taking
`RecordingMintFacts` (`{ runId?, label? }`); `RECORDING_ARTIFACT_KIND`
(`'recording/run'`) and `RECORDING_MEDIA_TYPE` (`'application/json'`) are the
constants a consumer branches on.
...
- **`s3Artifacts`** (`S3ArtifactsOptions`) and **`gcsArtifacts`** (`GcsArtifactsOptions`) put artifacts in **somebody else's bucket** — the third rung of the same ladder. Both partition scopes into percent-encoded **key segments** using the same encoding law as the directory adapter (a tenant of literally `..` is a *name*), carry the ticket as **one object-metadata entry** of ASCII JSON, store the payload as the **canonical bytes** of the object body (so a stored report is downloadable with the vendor's own console and *is* the report), and answer `null` only for a **missing object** — a denied, throttled, or misdirected call is an **error**, never "no data". A not-found is only "no data" when the call *asked about one object*: a 404 from a write or a listing means something the caller never asked about is wrong (most often that the bucket does not exist), so it raises with the SDK's own text withheld rather than being swallowed as an empty scope. Their statuses differ and are stated separately rather than averaged: **`s3Artifacts` is contract-shaped and tested** — no live call has been made against S3 from this repository — while **`gcsArtifacts` is field-validated**, an independent field trial having run nine live checks against a real bucket in 2026-08 (scope isolation, cursor pagination, native streaming, TTL expiry, budget eviction and the sanitized 404). One caveat travels with that: `@google-cloud/storage` is an optional peer whose tree carries five transitive **moderate** advisories, and the `npm audit fix --force` resolution is a major DOWNGRADE this library will not take — read the advisory on [Google Cloud](doc:google-cloud#cloud-storage--the-artifact-store-gcsartifacts-9250) before you inherit it. See [AWS](doc:aws) and [Google Cloud](doc:google-cloud) for keys, costs, and how to align the operator's lifecycle rules.
...
Six typed events (domain wildcard: `agent.on('agentfootprint.artifacts.*', …)`):

| event | payload | fires when |
|---|---|---|
| `agentfootprint.artifacts.minted` | `ArtifactMintedPayload` — ref, kind, bytes, origin, parents, `tool?` | a tool checked a payload in (or placement did it for one); `tool` is absent when no tool did — the run's own recording, or a host filing for its turn through `HostReply.turnArtifacts` |
...
| `agentfootprint.artifacts.hand_over_failed` | `ArtifactHandOverFailedPayload` — `cause` (`hook` / `operation` / `timeout` / `abort` / `expired`), `op?`, `errorClass?`, `errorCode?` | a host's own filing for a turn (`HostReply.turnArtifacts`) did not go cleanly — class only, stamped with its session; the reply was delivered regardless |
...
For custom wiring outside the Agent (your own harness, tests), the binding layer is public: `bindArtifacts(store, scope, options)` (a `BindArtifactsOptions` with `origin` + an `ArtifactEventSink` receiving each `ArtifactEventFact` — op vocabulary `ArtifactOp`, refusal vocabulary `ArtifactRefusalReason`, put shape `ToolArtifactPutInput`) and `unconfiguredArtifacts()`, the fail-closed teacher itself. `origin` is the binding's field on every path: a caller-supplied `origin` on a `put` is dropped, and the binding stamps its own — or none, when it was given none.
...
- **The reference architecture is complete as of 9.25.0:** typed HITL payloads landed in 9.24.0, and the cloud adapters, the optional streaming leg and the [skill artifact vocabularies](#skill-vocabularies--which-kinds-a-skill-makes-and-needs) landed here. Since 9.26.0 the data legs run **both ways**: [staging refs INTO code sessions](#the-code-leg--coderesultartifacts-gets-the-store-behind-it) landed with `CodeSession.stageInputs`, and [run recordings](#recordings-as-artifacts-9260--the-lens-over-the-wire) are artifacts the existing wire ops serve. **The lens leg shipped too, in a separate package** — `agentfootprint-lens` 0.34.0 carries the resolver, a component registry keyed on the artifact's `kind`, and a pane that renders an honest placeholder when a ref no longer resolves. Nothing validates `as`, because nothing selects on it; the names and the reasoning are in [Artifact architecture](doc:artifacts-architecture#chapter-4--the-screen).
