...
> **Deprecated for new procedures** — [`runbookAsTool`](doc:runbook-as-tool) is the standard bridge now: same chart-as-tool idea, but every answer carries the honesty envelope (coverage folded from inner tools, rule provenance, the recorded walk as an artifact ticket). `flowchartAsTool` stays supported for compatibility; `resultMapper` users stay put, and nothing here changes.
...
- [Tools guide](doc:tools) — the underlying Tool primitive
- [Tool providers](doc:tool-providers) — `gatedTools` / `skillScopedTools` chains
- [Output schema](doc:output-schema) — pair with `outputSchema` when the wrapped chart's result is a structured contract
