...
For the actual API — [`defineSkill`](doc:skills), [`SkillRegistry`](doc:skills), `AgentBuilder.skills()` — see the [Skills guide](doc:skills). This page is the **why**, that page is the **how**.
</Aside>
...
| **`Injection`** (agentfootprint) | agentfootprint | The one primitive every flavor reduces to: `{ trigger, inject, flavor }`. A per-iteration `trigger` decides inclusion; `inject` carries content to one or more slots. | `inject` targets the two slots an Injection can reach — `systemPrompt` and `tools`. (The third slot, `messages`, is the observability projection of the conversation; declaring content for it is refused.) |
...
`defineSkill({ id, description, body, tools })`. The library auto-attaches a `read_skill` tool to your agent's tool registry; the LLM activates the skill by calling it. When activated, the skill's body lands in the system slot for the rest of the turn. The skill's `tools` are registered up front and callable from iteration 1 — adding `autoActivate: 'currentSkill'`, as the sample below does, is what makes them appear only for that same window:
...
```ts
import { resolveSurfaceMode } from 'agentfootprint/context';
...
Belt-and-suspenders: even if system-prompt anchoring was honored at turn 1, by turn 40 on a non-Claude provider it's drifted out of effective attention. Re-injecting the body as a fresh tool result past a token threshold is the *idea* — but the hook never shipped, and it now never will in this form: `refreshPolicy` is **DEPRECATED-pending-steps** (9.16.0). The engine stores the field and ignores it; a planned steps-as-data feature owns re-delivery instead; dev mode warns once per process when the field is set. Today, `surfaceMode: 'both'` is the working way to re-surface a body (every `read_skill` call returns it afresh, recency-first).
...
└────────────────────────────────────────────────────────┘
         │
         ▼
  [ deprecated-pending-steps: refreshPolicy never wired; ]
  [ a steps-as-data feature will own re-delivery instead ]
```
...
A single skill is one playbook. Real agents have several — a front desk that hands off to billing or to tech support — and you want the handoffs to be **declared and drawable**, not buried in prompt prose. `skillGraph({ skills, start, steps })` is that: one object literal where `start` marks where a turn begins and each step `{ from, to, when }` declares a deterministic handoff. The SAME built object feeds the agent (`Agent.create()....skillGraph(graph)`) **and** draws as a graph — declared is drawn. (There is also a fluent builder that compiles to the identical graph — see [the fluent builder](doc:skills#advanced-the-fluent-builder) if you're migrating older code.)

The picture below is built from the exact `skillGraph({ ... })` shown above it — its boxes and edges are a build product of these `start` / `steps` declarations, not a hand-drawn diagram. Click a skill to see its playbook and the tools it unlocks.

<SkillGraphTryIt>
  <CodeFile path="docs-next/components/demos/skillGraphDemo.ts" region="demo" />
</SkillGraphTryIt>

Solid edges are the `steps` transitions you declared; dashed edges are skills the model can still reach on its own with `read_skill`. The graph is also the single source of two runtime guarantees: `graph.nextSkill(ctx)` advances the cursor exactly as the drawn routes say, and `graph.reachableSkills(ctx)` is the gate that stops the model from `read_skill`-jumping outside the graph. One object — the agent runs it, the docs draw it.
...
- **Wondering why your framework's "Skills" feature doesn't compose with your instruction system** — because they're the same thing. Skills are `Injection`s (the same primitive `defineInstruction` builds) with a discovery layer bolted on. If your framework treats them as separate primitives, you're paying for parallel abstractions.

## See also

- [Skills](doc:skills) — the API reference
- [Instructions & decisions](doc:instructions) — the `Injection` primitive (via `defineInstruction`) Skills compose over
- [Tool use](doc:tools) — the protocol Skills ride on
