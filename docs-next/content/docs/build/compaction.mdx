...
| `WindowRefusalReason` | The closed set of reasons a turn did not fold: `system-envelope`, `current-request` (9.55.0), `last-tool-result` (9.57.0), `unresolved-tool-call`, `paused-tool`, `pending-check-in`, `inside-keep-window`, `only-existing-summary`, `summarizer-failed`, `replacement-not-smaller`. Branch on it; it grows in a minor, never silently. (`FoldRefusalReason` was the 7.16 name; the alias was removed in 9.0.0, along with the member `summary-not-smaller`.) |
| `WindowObservations` | What the last-tool-result pin did on one visit: which turns it held (tool, turn index, exact chars), how many the ceiling turned away, and `standDown` when it released. On `WindowRecord.observations`. |
