---
title: Embedders
description: openaiEmbedder, localEmbedder and staticEmbedder — ready-made Embedder implementations. What each one needs, which ones run in a browser, and what they cost.
---

> Every semantic feature in agentfootprint — [semantic retrieval](doc:semantic-retrieval), [RAG](doc:rag), `toolChoiceRecorder`'s relevance scoring — needs one thing from you: an **embedder**, something that turns text into a vector. The core ships only `mockEmbedder()` so it stays dependency-free. `agentfootprint/embedders` is where the three real ones live.

## The three

All three satisfy the same `Embedder` contract, so they are interchangeable at
...
Both peer deps are **optional**: installing agentfootprint does not install
either one, and neither is loaded unless you actually call the factory that
needs it.

The type they all return is exported from the same subpath:

```ts twoslash
import { staticEmbedder, type Embedder } from 'agentfootprint/embedders';
...
```ts twoslash
import { openaiEmbedder } from 'agentfootprint/embedders';
// ---cut---
...
```ts
import { localEmbedder } from 'agentfootprint/embedders';
...
```ts twoslash
import { staticEmbedder, type Model2VecBackend } from 'agentfootprint/embedders';
...
```ts twoslash
import type { Embedder } from 'agentfootprint/embedders';
