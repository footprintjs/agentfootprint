...
- **A blank label is not a meaning** — `label: ''` is recorded as no entry, for a rule or for the default.
...
**Redaction means the same for both.** The recording's snapshot is read from the *redacted mirror*, not raw working memory — so the `redact` policy that scrubs the walk scrubs the recording by the same rule at the same moment. A redacted key is still *present* in the recording, carrying `REDACTED`: a reader sees that a value existed and was scrubbed, rather than that nothing was written. Since footprintjs 9.19.0 (agentfootprint 9.89.2 requires `^9.19.1`) that mirror and the log beneath it are clean through every path — a subflow's seed and merge-back, tracked reads, dot-path `fields`, the narrated `Input:` line — so the recording is exactly as clean as the policy says. The last surface that view left raw — a subflow's own final heap, which agentfootprint refolded from the subflow's scrubbed history before filing — is closed by footprintjs 9.20.0 (agentfootprint 9.89.3 requires `^9.20.0`): a subflow keeps its own redacted mirror whenever the run does, and the recording carries that mirror exactly as footprintjs serves it, with nothing refolded here. Nothing left the log carries that the served view does not scrub. The live state the chart computes on and the resume checkpoint are never scrubbed, by law: resumption must replay real values — the checkpoint is not a served view.
...
- **The two render laws** — `RunbookPresentation` (`'prose' | 'panel'`) is the `presentation` dial's type; `VERDICT_RENDER_NOTE` is the law stated beside a pre-rendered table under `'prose'`, and `PANEL_RENDER_NOTE` is the one stated instead of a table under `'panel'`.
