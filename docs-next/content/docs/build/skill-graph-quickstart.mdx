...
- `SkillToolDescriptor` is a tool the graph *describes* rather than builds.
  ...
  what the gate will actually grant. A host wiring its own `read_skill` under
  a decision `tree()` must set `treeRouted: true` on it: a tree routes by
  predicate on every iteration and keeps no cursor, so every routing pick is
  refused by construction, and the flag is what lets the descriptor withhold
  the offer (with open skills present it explains the tree and lists what a
  pick can open instead). Without it the host keeps offering a menu the graph
  will reject.
