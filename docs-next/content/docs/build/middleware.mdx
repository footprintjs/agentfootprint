...
Middleware is written into [`.act()`](doc:loop-moments), the one block that says what an agent does at each moment of its loop:
...
The individual doors (`.toolMiddleware()`, `.messageMiddleware()`) are still there and still supported — they are what `.act()` forwards to — and they are the right spelling when you are adding one rule to an agent somebody else built. See [Composing incrementally](doc:loop-moments#composing-incrementally).
...
| `allow(undefined, why)` | pass it through, and say why you were comfortable | nothing moves; the row carries the reason |
...
A tool rule may speak twice: `onToolCall` about the CALL, and `onToolResult` about the RESULT once the tool has run. Those two halves are one chain walked in [onion order](doc:loop-moments#the-onion), and the second has its own page section — [the after-tool moment](doc:loop-moments#the-after-tool-moment) — because its laws are different: two verbs, no `ask`, and it never runs for a call that never executed.
...
The checkpoint is JSON. It is the same pause machinery `askHuman` and `checkIn` ride — not a second one — which is why it already works everywhere they do, including [hosting](doc:hosting-and-runtime), where an outstanding question is refused as unfinished work rather than silently dropped.
...
That holds on **every** turn. A continued conversation — `run({ message, continueFrom })`, `followUp()`, a stored session behind `standingAgent` — appends the message the chain let through, so the stored history keeps the rewritten turn, the same string `checkpoint().originalInput.message` names. (Before 9.112.2 the chain ran on a continued turn but its output was discarded: the model was sent, and the conversation stored, the raw message, while the ledger row and `userMessage` held the rewrite.)
...
**An `'input'` scrub keeps the text away from the model and the conversation — not from every record of the run.** On every turn the chain lets through (first or continued, since 9.112.2) the model reads the scrubbed message, the committed `history` and `checkpoint().history` hold the scrubbed turn, and no `agentfootprint.*` event payload carries the original. The run still keeps what it replaced — that is the honest trade that makes the transform legible — so the pre-scrub text is **not** in one place. It is in these, and an app that must not keep it redacts each one itself before it persists or serves it:

| holds the pre-scrub text | where it shows up |
| --- | --- |
| the ledger row, `middlewareDecisions[i].before` | every copy of the run's state: `getLastSnapshot()` (`sharedState`, `commitLog`, `executionTree`, `subflowResults`), `getLastNarrativeEntries()`, a `recordRun(...)` recording, a `BoundaryRecorder` `subflow.entry` payload — and a paused run's checkpoint, `RunnerPauseOutcome.checkpoint` (`sharedState`, `executionTree`), which `standingAgent` itself stores for a paused session as a `flowchart-v1` envelope under **every** durability, the default `'exit'` included |
| the run's input, as passed | the `run.entry` payload every flow recorder receives — `BoundaryRecorder`, `recordRun`, a Lens — is `{ message }` before the chain ran |
| a crash checkpoint | `RunCheckpointError.checkpoint.originalInput.message` is the message as passed (`checkpoint().originalInput.message` is the chain's verdict) |
| a refused turn | `deny` commits the content as it stood when refused — the raw text, unless an earlier link rewrote it — into `history`. `checkpoint()` after the refusal carries it, and a `followUp()` or a `standingAgent` session under `durability: 'async'` or `'sync'` (any mode except the default `'exit'`) sends it to the model on the next turn |

The last two are known gaps, recorded in the 9.112.2 changelog entry, not the design. Until they close: replace a crash checkpoint's `originalInput.message` with the scrubbed text before you store it (safe for a scrub: resuming runs the chain again and it finds nothing to change), and continue a refused conversation from a checkpoint with the refused entry removed — or keep `standingAgent` on its default `durability: 'exit'`, which does not carry a refused turn into the next one (`'async'` and `'sync'` both do). `'exit'` does not keep a paused run's checkpoint out of the store, though — that copy is covered below.

If your threat model says the original must not survive in a record, that is a **redaction** question, not a middleware one. footprintjs's redaction policy answers it at write time — the decision row survives, so the run still says a scrub happened, who did it, and why, without holding what was scrubbed. **An `Agent` does not expose that policy today** (`flowchartAsTool` and `runbookAsTool` take `redact`; an Agent's executor is built without one), so for an Agent the scrubbing happens where you keep the record: strip `before` / `after` from the `middlewareDecisions` rows and the `run.entry` payload before a snapshot or a recording leaves the process. A paused run's checkpoint needs the same strip before it is stored: if you store `out.checkpoint` yourself, strip the rows in its `sharedState` and `executionTree` first; behind `standingAgent` the library stores it, so wrap the store you pass as `sessions` — its `persist` strips those rows from a `flowchart-v1` envelope's `data.checkpoint` and then writes. The ledger is only ever appended to, so a checkpoint stripped this way resumes the same. For a local-observability `Trace`, `serializeTrace(events, { redact: redactContent })` blanks the `run.entry` / `subflow.entry` payloads at the serialize boundary. Honesty and protection are both laws here, and that is where they meet.
...
The `'after-tool'` moment is the input phase's mirror image: a result the model was **not** allowed to read is kept in that row's `before` — and, by design, in the `agentfootprint.stream.tool_end` event, which reports what the tool returned while the history carries what the rules let through. Same trade, same answer as above: redact both where you keep the record and the refusal survives without the value.
...
A tool NAME is not an identity. Two [MCP servers](doc:mcp-client) can each serve a `call_aws`, and a rule matching the bare name governs whichever one answers — including the one it was never written about. So the context carries `toolSource`: the name of the server the tool came from.
...
| `ToolMiddlewareContext` | What a tool link is handed: `toolName`, `toolSource?`, `toolCallId`, `iteration`, `args`, `history`, `identity?`, `signal?`. |
| `ToolResultContext` | The same, plus `result` and `error?` — what `onToolResult` is handed. `args` are what the tool actually ran with. |
...
| `ToolResultOutcome` | `allow \| deny`. No `ask` — the tool has already run. |
| `MessageOutcome` | `allow \| deny`. No `ask` — the message boundary has no pause to carry one. |
| `MiddlewareDecision` | One ledger row, stamped with the `moment` it came from. |
| `LoopMoment` | `'input' \| 'before-tool' \| 'after-tool' \| 'window' \| 'output'` — see [the moments](doc:loop-moments). |
...
- [Connecting to an MCP server](doc:mcp-client) — where `toolSource` comes from
- [The moments of the loop](doc:loop-moments) — `.act()`, the after-tool moment, and the whole table
