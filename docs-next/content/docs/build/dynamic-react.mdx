...
**Read the code, hit Run, watch it trace.** The exact agent below runs **right in your browser** — a mock LLM, no network, no server. You'll see the real ReAct loop live, iteration by iteration: `read_skill(billing)` → `redact_pii` → the on-tool-return reminder fires → `process_refund` → a final answer with the email redacted to `[EMAIL]`.

<DynamicReactTryIt>
  <CodeFile path="docs-next/components/demos/dynamicReactDemo.ts" region="demo" />
</DynamicReactTryIt>

> **What to watch.** The flowchart's *shape* is identical on every iteration — the agent is a fixed set of stages, so no new boxes appear as it loops. The dynamism is two things: **(1)** which boxes *light up* as the cursor advances, and **(2)** the **Context engineering** list in the right-hand panel. Scrub to the iteration right after `redact_pii` returns and you'll see the `post-pii` reminder show up in the system-prompt slot — *with the reason it fired* — and nowhere else. Same boxes, different context.
...
This is the **on-tool-return** trigger from the [4-trigger taxonomy](doc:key-concepts#context-engineering-cross-cutting). It's the recency-first injection point: the LLM's attention is freshest on the most recent message, and a tool result IS a recent message.
...
Here is that same agent drawn as **raw footprintjs subflows** — no agent vocabulary, just the plain stages the executor traverses. The three context slots (`sf-system-prompt` ∥ `sf-messages` ∥ `sf-tools`) are a real parallel fan-out; `sf-injection-engine` decides what each one composes. Drill into a box to see its internals. This is the "library of libraries" layer underneath the agent — the agent abstraction is *built on* these footprintjs subflows.

<SubflowLensTryIt />

> **v2.5 fix note:** v2.4 had a regression where the loop target skipped InjectionEngine on iter 2+, quietly disabling per-iteration re-evaluation. v2.5 (commit pending) restored the v1 behavior. Tests added at `test/core/dynamic-react-loop.test.ts` pin the per-iteration evaluation contract — `on-tool-return` predicates now fire on iter 2+, `autoActivate` per-skill tool gating works (it rests on the tool list being recomposed every iteration), and skill bodies inject on the iteration after `read_skill('X')` activation.
...
const summarizeOnLargeResult = defineInstruction({
  id: 'summarize-large',
  activeWhen: (ctx) => (ctx.lastToolResult?.result?.length ?? 0) > 5000,
...
The LLM activates `billing` via `read_skill('billing')` → next iteration's tool list flips from "all 25 tools" to "the 7 tools billing actually uses". 3× context-budget reduction + sharper LLM tool-choice. This is what `defineSkill({ autoActivate: 'currentSkill' })` buys, and the Agent wires it for you — **without that option a skill's tools are in the tool list from iteration 1 and activation changes nothing about them.** Outside the Agent's own wiring, compose the same gate with `skillScopedTools(id, tools)` from `agentfootprint/providers`.

### 3. Length guardrails

The loop runs long → next iteration's system prompt adds *"be concise; final answer in 2 sentences"*. Pure predicate over the read-only `InjectionContext` (`iteration` / `history` / `lastToolResult` / `toolResults` / `userMessage` / `activatedInjectionIds`); no separate management subsystem needed.

<Callout type="info">
**Parallel batches (9.16.0).** When the model calls several tools in one
message, `on-tool-return` triggers match against **every** result of that batch
— not only the last call, which was the pre-9.16.0 behavior. A predicate that
wants the whole batch reads `ctx.toolResults` (call order, `toolCallId`
included); `ctx.lastToolResult` remains the last entry, so existing predicates
...
defineInstruction({
  ...
  prompt: 'This turn is running long. Wrap up — final answer in 2 sentences.',
});
```

For true cost-based guardrails, track spend with `costRecorder()` from `agentfootprint` and gate the run outside the predicate — the `InjectionContext` does not expose accumulated cost.
...
System-prompt skill body decays past long contexts (~50K tokens on most providers) → re-inject via tool result so the LLM sees it fresh again. `defineSkill({ refreshPolicy: { afterTokens: 50_000, via: 'tool-result' } })` is where that intent is declared — but it is **declared only**: nothing in the engine reads `refreshPolicy` yet, so no re-injection happens today. Reach for `surfaceMode: 'both'` if you need the body returned on every `read_skill` call.
...
- [Instructions guide](doc:instructions) — the broader Instruction primitive (rule + on-tool-return + always)
- [Skills, explained](doc:skills-explained) — LLM-activated body + tools (the cousin pattern)
