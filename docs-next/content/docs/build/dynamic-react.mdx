...
This runs the **exact example below** (`examples/context-engineering/05-dynamic-react.ts`) **right in your browser** — a mock LLM, no network, no server. Hit **Run** and watch the real ReAct loop trace live, iteration by iteration: `read_skill(billing)` → `redact_pii` → the on-tool-return reminder fires → `process_refund` → a final answer with the email redacted to `[EMAIL]`.
