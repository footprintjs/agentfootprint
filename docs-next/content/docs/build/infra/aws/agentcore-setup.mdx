...
npm install @aws-sdk/client-bedrock-agent-runtime     # AgentCoreStore (memory)
npm install @aws-sdk/client-bedrock-agentcore         # agentCoreIdentity
...
`put/get/list/delete/forget` map to `CreateEvent/GetEvent/ListEvents/DeleteEvent/DeleteSession`;
the `identity.conversationId` is the AgentCore **sessionId**. (Server-side semantic retrieve,
`RetrieveMemoryRecords`, isn't wrapped yet — use `WINDOW` / `TOP_K`.) Deep dive:
[Memory store adapters](doc:agentcore).
