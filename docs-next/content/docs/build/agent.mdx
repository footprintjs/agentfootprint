...
There are no extra model calls, and an agent that never calls `.findings()` records exactly the bytes it recorded before. What the model is served does not change yet; the ledger is on the record first.
