...
- [Observability guide](doc:observability) — the broader event taxonomy
- [Dynamic ReAct guide](doc:dynamic-react) — what makes per-iteration recomposition produce a stream of engineered injections
- [RAG](doc:rag), [Memory](doc:memory), [Skills](doc:skills), [Instructions](doc:instructions) — the engineered-source primitives
