...
There are **73 typed events across 20 domains**: `agent` (incl. `output_schema_validation_failed` for Instructor-style retry and `output_schema_retry` for the loop-level re-ask), `composition`, `context`, `stream`, `tools`, `skill`, `memory`, `cost`, `permission` (incl. `halt` for sequence governance), `credential` (declare-and-push resolution), `eval`, `embedding`, `pause`, `checkin` (evidence-carrying human consent), `middleware` (a governance chain answered), `error`, `reliability`, `fallback`, `risk`. Every event has a typed payload — `agentfootprint.stream.llm_start`, `agentfootprint.stream.tool_start`, `agentfootprint.cost.tick`, `agentfootprint.composition.iteration_start`, etc. The framework owns the loop and emits during DFS traversal — no instrumentation, no spans-after-the-fact.
