...
2. **Exposes sync `isAllowed(toolId)`** — pair it with `gatedTools` from [agentfootprint/tool-providers](doc:tool-providers) to filter the tool list at composition time:
