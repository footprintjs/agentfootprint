...
### The answer's typed half — `DecisionValue`

A yes is not always what the person gave you. When the ask was "pick one", the answer is a **value**, and `DecisionValue` is the shape it rides back in: `kind` (your vocabulary for what the value IS — `'row-choice'`, never a type name from this library), `value` (small, clone-safe inline JSON, because it lands in the run's history), `from` (the artifact the choice was made **against**, when there was one), and `coverage` (`seen` of `total`, plus the `filter` that narrowed it). It rides `checkInApproved({ by, value })` and `checkInDeclined({ by, value })` — see [Check-in](doc:checkin) — and is optional everywhere, so a screen that only approves or declines never sets it.

`coverage` is the field people skip and the one that carries the point: a person who filtered 5,000 rows to 3 and picked one **has not chosen from 5,000**. Without it that pick is indistinguishable from an informed choice over the whole set, and the difference is the whole value of having a human in the loop. It is the same distinction `coverage()` draws for a tool — what was checked, and what a clean answer does not rule out — applied to the person, because on this turn the person *is* the tool.

**An answer chosen against different bytes than the question was asked against is refused.** When the ask pinned a `propsRef` and the answer names a `from`, and the two disagree, `agent.resume()` raises `StaleDecisionError` (`code: 'ERR_STALE_DECISION'`, carrying both `asked` and `answered`) — nothing is resumed and the checkpoint is unchanged. This happens for an ordinary reason: time passes between asking and answering, a refresh lands, a filter moves, rows re-sort, and "the third row" now names something else. Accepting it would resume the run with a value the person never chose, and nothing downstream would ever signal it — the id is well-formed, the type checks, the loop continues. A person asked again is mildly annoyed; a person whose gesture was silently reinterpreted has been misrepresented, and in a consent record that is the one failure nothing can recover from. It fires **only** when both sides named an artifact: an approve/decline, a plain `askHuman` value, or an answer that names none are all byte-identical to every earlier release.
