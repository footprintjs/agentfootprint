...
[Context Integrity](doc:context-integrity) ships three checks. None of them runs for free. `invariant-violation` at the compose seam needs a mounted map, `dangling-reference` needs a tool that says where its arguments come from, `unsupported-claim` needs a claim contract — and until 9.62.0, a check with no precondition declared filed **no row at all**. The one always-on row (`wire`) sat there green next to nothing, and a run that checked three things looked identical to a run that checked one.
...
Three of these are one field. The fourth (`invariant-violation` at `compose`) is one call — `.maps()` with no arguments — but only once a skill map already exists; if nothing is mounted yet, `.maps()` refuses at `build()` rather than mounting a kernel over an empty set, because there would be nothing for it to park.
...
Here is a real-shaped report — an agent that declared `argumentsFrom` on one tool and nothing else (no `.maps()`, no `.claims()`), after a short run where one call actually hit a dangling reference:
...
The finding type names five defect classes: `invariant-violation`, `unsupported-argument`, `dangling-reference`, `duplicate-execution`, `unsupported-claim`. **Three ship as checks.** `unsupported-argument` (acted on a value nothing served) and `duplicate-execution` (did settled work again) have no implementation in this build — no code anywhere is looking for either one.

That absence is stated, not hidden. `find_context_errors` — the tool a debugging agent calls to read findings back from a recording — never offers those two kinds in its schema, and if something calls it with one anyway, it answers with an explicit refusal rather than a clean-sounding negative:

> `find_context_errors: ⚠ UNANSWERABLE — 'duplicate-execution' is a defect class the finding type names, and no check in this build can file it. Nothing ever looked for one, so silence about it is not evidence of its absence. Classes this build can file: invariant-violation, dangling-reference, unsupported-claim — call again with one of those, or with no 'kind' at all.`

The alternative — silently returning "no duplicate-execution findings" — would be a false negative wearing the same green as a check that actually looked and found nothing. A doc page (or a tool response) that implies coverage that doesn't exist is exactly the disease this whole family exists to cure, so it refuses instead. If your agent's real risk is duplicate tool execution or an argument value nobody ever served, arming the three shipped checks buys you nothing there yet — that gap is open, named, and unclaimed by any declaration in this doc.
