...
Every detected defect is one `agentfootprint.integrity.context_error` event — one per defect per run, however many passes re-detect it. The payload is the finding: `kind` (`'invariant-violation'` | `'dangling-reference'` | `'unsupported-argument'` | `'unsupported-claim'` today), `seam` (`'compose'` | `'wire'` | `'choice'` | `'claim'`), `subjects` (who it is about), `witnesses` (both channels' assertions — neither preferred), `message` (plain sentence naming the fix). A finding may also carry `predicate` — WHICH relation the defect is about, when the subjects alone do not say (the claimed field, the argument's dot-path); it is part of the finding's identity, so two bad arguments of one call are two defects rather than one. A finding may also carry `advisory: true` — a seam saying "this is not a defect" (the model declining to claim a verified fact is doubt, not contradiction); advisories are counted apart from defects everywhere the family reports. Example:
