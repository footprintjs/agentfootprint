...
- `ReliabilityConfig` — the rule list shape (see [`agentfootprint/resilience` types](https://github.com/footprintjs/agentfootprint/blob/main/src/reliability/types.ts))
- `ReliabilityFailFastError` — thrown at `agent.run()` on `fail-fast` decisions; carries `kind`, `reason`, `cause`, `payload`, `snapshot`
- `initialBreakerState` and the `BreakerState` / `CircuitState` types — circuit-breaker primitives exported from `agentfootprint/resilience` for consumers hydrating breaker state from a persistence store
- `CircuitOpenError` — **the one name that did NOT move to the door.** The gate throws a different class than the [provider decorator](doc:resilience) does; `agentfootprint/resilience` carries the decorator's. If you `instanceof`-check the error the GATE throws, import it from `agentfootprint/reliability`, which stays available for all of 8.x
- Schema-retry helpers (v2.13) — `ValidationFailure`, `lastNValidationErrorsMatch`, `defaultStuckLoopRule`, and the `OutputSchemaValidator` type, also from `agentfootprint/resilience`. Pair with a rule's `feedbackForLLM` to run Instructor-style retry that tells the model what to fix, and short-circuit stuck loops where it keeps making the same mistake.
