...
- [Error handling guide](doc:error-handling) — typed errors at the SDK boundary; precursor to the Reliability primitives
- [Observability guide](doc:observability) — wire `onStateChange` / `output_canned_used` / `failurePoint.phase` into your observability backend
