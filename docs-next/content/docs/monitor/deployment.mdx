...
|---|---|---|
| LLM provider | `mock({ reply })` | `anthropic({...})` · `openai({...})` · `bedrock({...})` · `gemini({...})` |
| Embedder | `mockEmbedder()` | OpenAI / Cohere / Bedrock embedder factory (on the roadmap) |
| Memory store | `InMemoryStore` | `RedisStore` · `AgentCoreStore` (both from `agentfootprint/memory`) |
...
The vendor-SDK provider factories live on the `agentfootprint/providers` subpath — the main barrel only exports the zero-peer-dep providers (`mock`, `browserAnthropic`, `browserOpenai`, `createProvider`):
...
- **Cost tracking** — pass `pricingTable` + `costBudget` to `Agent.create()` (add `onExceed: 'halt'` to make the budget stop the run rather than warn)
...
- **Cost-budget enforcement** — pass `pricingTable` + `costBudget` to `Agent.create()`. A bare number emits cost ticks and a one-shot `cost.limit_hit` and keeps going; `costBudget: { usd, onExceed: 'halt' }` (8.14.0) stops the loop at the next iteration boundary. Read `agent.stoppedEarly()` to tell the two apart after the run.
...
- [Memory store adapters](doc:memory-and-stores) — production backend matrix
