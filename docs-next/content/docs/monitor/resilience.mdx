...
Inside an `Agent`, `LLMCall`, or `Parallel` run you get decorator telemetry **for free** — no wiring. The decorators report what they did, and the LLM call site turns each report into a typed event stamped with the real `runId` and `runtimeStageId`, so it correlates with everything else in the run:

| Event | Fired by | Payload |
| --- | --- | --- |
| `agentfootprint.fallback.triggered` | `withFallback` | `{ kind: 'provider', primary, fallback, reason }` |
...
```ts
const provider = withFallback(anthropic({ apiKey: A }), openai({ apiKey: O }));
const agent = Agent.create({ provider, model: 'claude-sonnet-4-5' }).build();

// Subscribe BEFORE run() — events are dropped when nothing is listening.
agent.on('agentfootprint.fallback.triggered', (e) => {
  console.warn(`${e.payload.primary} → ${e.payload.fallback}: ${e.payload.reason}`);
});
// Or take the whole family:
agent.on('agentfootprint.error.*', (e) => console.log(e.type, e.payload));

await agent.run({ message: 'hi' });
```

Notes on what these events do and don't say:

- **`error.retried.reason` classifies the ERROR, not the decision.** `shouldRetry` returns a bare boolean, so a custom predicate's reasoning is unknowable. The value is derived from the error's `status`/`statusCode`: `'http-429'`, `'http-5xx'`, `'http-4xx'`, `` `http-${code}` ``, or `'no-status'`.
- **`withCircuitBreaker` has no event of its own.** A trip shows up as the `reason` on the enclosing `withFallback`'s `fallback.triggered` (the `CircuitOpenError` message). Use `onStateChange` for standalone breaker state.
- **A fallback success is reported as `fallback.triggered`, not `error.recovered`** — that payload has no field for which provider served, so claiming a recovery there would be inventing data.
- Running one of the exported message-api charts on a bare `FlowChartExecutor`? Attach the bridge yourself: `executor.attachCombinedRecorder(resilienceRecorder({ dispatcher, getRunContext }))` from `agentfootprint/observe`.

## Hooks for standalone use

Outside a run the decorators emit nothing, so the optional hooks remain the way to observe them — useful in a plain script or a non-agentfootprint call path. `onRetry` receives the upcoming attempt number and the computed backoff delay in ms:
