...
import { mockMcpClient } from 'agentfootprint/tool-providers';
...
- [Quick Start § mocks-first](doc:quick-start) — the workflow recommendation
- [Tool discovery](doc:tool-discovery) — `mockMcpClient` shape + production swap to `mcpClient`
- [Memory guide](doc:memory) — `InMemoryStore` for ephemeral test scenarios
