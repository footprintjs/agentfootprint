...
```ts
import { chatBubbleLiveStatus } from 'agentfootprint/observe';
...
```ts
import { consoleObservability } from 'agentfootprint/observe';
...
102 typed events across 22 domains. Your IDE auto-completes payload shapes per event name. See [Observability guide](doc:observability) for the full taxonomy.
