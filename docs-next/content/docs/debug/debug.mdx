...
65 typed events across 18 domains. Your IDE auto-completes payload shapes per event name. See [Observability guide](doc:observability) for the full taxonomy.
...
Every run persists a snapshot. Two weeks later, when the customer comes back asking why, the matching snapshot loads into the next prompt — the follow-up answers from the stored run, not a rerun. See [Memory guide § Causal](doc:memory#causal-memory--replay-decisions-not-just-messages).
...
- [Localize a context bug (Beta)](doc:localize-context-bug) — git bisect for context: find and *prove* which input caused a bad output
- [Observability guide](doc:observability) — the full event taxonomy
- [Memory guide](doc:memory) — Causal memory for cross-run debugging
- [Testing guide](doc:testing) — mocks-first means most bugs surface in CI, not prod
