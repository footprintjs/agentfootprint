...
If contextual errors are the new class of bug ([Why agentfootprint?](doc:why#the-new-class-of-bug)), then the trace is the recording that makes them debuggable. agentfootprint's observability hands the trace back as three workflows — Live, Offline, Detailed.
...
The snapshot maps directly onto the four backtrack questions every agentfootprint LLM call answers (see [Why § the four backtrack questions](doc:why#the-four-backtrack-questions)):
...
- [Memory guide](doc:memory) — the full type × strategy matrix that includes Causal
- [Auto memory (Hybrid)](doc:auto-memory) — stack causal alongside recent + facts
- [Observability guide](doc:observability) — the three workflows the trace also enables
- [Citations & papers](doc:citations) — the research foundation
