...
<Diagram name="causal-memory" alt="One agent run produces a JSON-portable causal trace. Three downstream consumers fan out (audit replay, cheap-model triage, training data export). The fourth, novel: the agent reads its own trace via causal memory." />
...
import { defineMemory, MEMORY_TYPES, MEMORY_STRATEGIES, SNAPSHOT_PROJECTIONS } from 'agentfootprint/memory';
import { anthropic } from 'agentfootprint/providers'; // vendor-SDK providers live on this subpath
