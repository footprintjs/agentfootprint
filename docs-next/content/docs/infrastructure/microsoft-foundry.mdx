...
- **The Foundry endpoint beats a leftover Azure key.** The credential arms
  trigger on a *credential*, and credentials linger in a shell by accident;
  `FOUNDRY_PROJECT_ENDPOINT` is a product-specific spelling nobody exports by
  mistake, so where it is present Foundry is the declared destination.

`FOUNDRY_PROJECT_ENDPOINT` with no deployment named is refused rather than
guessed — Foundry routes by deployment and there is no default — and the refusal
names the variable to set. Detection is **env-only**: no socket is opened to see
