...
per-request proof — and `GatewayTransportOptions` can vend headers only. There is
no certificate, no signer and no `fetch` hook here, so this transport is **not**
the path for Google's identity-enforced traffic. The next section is: build the
signer on the generic `http` transport's `fetch`. See
