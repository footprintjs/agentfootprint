...
**A session has two doors, and confusing them costs a day.** `BrowserSession`
...
exactly `mouseClick | mouseMove | mouseDrag | mouseScroll | keyType | keyPress |
keyShortcut | screenshot`. **There is no navigate action.** That is not a gap in
...
`handControlTo` takes a `BrowserDriver` — `'person'` or `'agent'` — and is
...
handover, the wait and the resume are ordinary events in the trace, so "why did
this run take four minutes?" has an answer that names a person and a login
screen.
