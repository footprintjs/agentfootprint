...
[`examples/features/03-permissions.ts`](https://github.com/footprintjs/agentfootprint/blob/main/examples/features/03-permissions.ts)
...
1. a read-only role passing the tool-name allowlist and still failing the
   `memory_write` capability the tool declares — **zero** executions;
2. an admin role clearing `tool_call`, `memory_write` and `user_data` — one;
3. a checker throwing `simulated authorizer outage` — fail-closed, zero
   executions, the operator's error in the `rationale`, the model carrying on
   without the tool;
4. a `halt` decision — the run ending with `PolicyHaltError` carrying both
   vocabularies above.

<Callout type="info" title="Independently reproduced against a local harness — the gate is an execution guard, 2026-08-13">
...
The reviewer's own summary is worth keeping verbatim: *"The permission checker is
a real execution guard, not merely tool hiding."* Its caveat is kept too — that
holds when the policy inputs and the role binding are themselves trustworthy,
which is a fact about your deployment rather than about this port.
