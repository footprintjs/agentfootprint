...
| `firestoreSessions` — a `SessionLifecycle` on Firestore | — | **Tracked ticket, not scheduled here.** An independent field trial's own adapter passed a live multi-user run against a real Firestore (2026-08-13) and proved the 9.26 ownership CONTRACT — but a shipped one needs a new optional peer (`@google-cloud/firestore`), an SDK pin test before any adapter code, an indexed `orderBy` + document cursor (the trial's `listByUser` sorted client-side), a TTL policy and a write-once owner. Its own release. The port is two methods meanwhile |
