...
<Callout type="warn" title="Status: contract-shaped and tested — NOT field-validated">
**Nothing here has been run against a live Firestore by this repository.** That
sentence is the status, not a hedge attached to one, and no amount of green CI
changes it.

Every SDK member this adapter calls was read off a real
...
<Callout type="info" title="What a field trial of a DIFFERENT adapter did and did not establish">
The design is informed by an independent field trial, 2026-08-13, of a
...
**One collection, one document per session, six fields** — `sessionId`,
`format`, `savedAt`, `envelope`, `owner`, `messageCount`. That is deliberately
...
complaint about "an odd number of components".
...
`{ project?, database?, collection?, firestore? }`. `project` and `database`
...
| `firestoreSessions()` — a `SessionLifecycle` on Firestore | `agentfootprint/hosting` | **9.33.0 — contract-shaped and tested; NOT field-validated.** Nothing here has been run against a live Firestore by this repository; the eighteen pinned SDK members were hand-verified against a real `@google-cloud/firestore` 9.0.0 install outside the repo, and the reality assertion **skips** in CI while the dispatch pin runs everywhere. The design is informed by an independent field trial of a *different* adapter (2026-08-13) that proved the ownership and history contract against a real Firestore — and that had no indexed query, no cursor, no transaction and no index requirement, so it says nothing about this one. [Full account](#firestore-sessions--firestoresessions) |
