...
and `gcsArtifacts` (see [AWS](doc:aws) / [Google Cloud](doc:google-cloud)) work
...
verbs. See [Artifacts](doc:artifacts).
...
What that does **not** claim: sustained production traffic, rotation behaviour at
scale, or a disk-full path. Those are still the tests' word, not the field's.
</Callout>
