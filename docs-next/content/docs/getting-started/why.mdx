...
> 📖 [Dynamic ReAct guide](doc:dynamic-react) for the full taxonomy of what this unlocks (tool-by-tool steering, adaptive tool exposure, cost guardrails, iterative format refinement, failure adaptation).
...
- [Quick Start](doc:quick-start) — build your first agent
- [Key Concepts](doc:key-concepts) — the 5-layer taxonomy
- [Reliability gate](doc:reliability-gate) — rules-based retry / fallback / fail-fast around every LLM call
- [Causal memory deep dive](doc:causal-deep-dive) — how the trace becomes the agent's working memory
