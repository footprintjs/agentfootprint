...
description: AI agents fail in a way classic debuggers can't see — the code is right, but the answer is wrong because of the context the model saw. agentfootprint records that context so you can trace any wrong answer back to its cause.
---
...
<Callout type="warn" title="The model doesn't tell you which of these went wrong">
...
<Diagram name="causal-memory" alt="One agent run produces a JSON-portable causal trace. Three downstream consumers fan out: audit replay, cheap-model triage, training data export." />
...
<Diagram name="dynamic-vs-classic" alt="Classic ReAct loops back to CallLLM (slots frozen). Dynamic ReAct loops back to SystemPrompt (slots recompose every iteration)." />
