...
If you haven't yet, read [Why agentfootprint?](doc:why) first. It's the *why*. This page is the *what*.
...
<Diagram name="dynamic-vs-classic" alt="Classic ReAct loops back to CallLLM (slots frozen, 12 tools every iteration). Dynamic ReAct loops back to SystemPrompt (slots recompose, tools shrink from 1 to 5 as skills activate)." />
...
> 📖 [Dynamic ReAct guide](doc:dynamic-react) for the full taxonomy of what this unlocks.
...
<Diagram name="control-flows" alt="The 4 control flows: Sequence (linear chain A → B → C), Parallel (fan-out + fan-in), Conditional (diamond branch), Loop (cycle back)." />
...
**Use for:** multi-step workflows, classify-then-respond, data (ETL) pipelines.
...
<Diagram name="triggers" alt="Every LLM call has 3 fixed slots (system, messages, tools); every flavor lands in one slot under one of 4 fixed triggers. The grid is the entire context-engineering surface." />
...
<Callout type="info" title="Slot is a default, not a coupling">
...
Those four answers are why contextual errors stop being invisible. See [Why agentfootprint?](doc:why#the-four-backtrack-questions) for the full bug-class argument.

### Opt-in outer JSON context meanings

`CONTEXT_FIELD_MEANINGS` is an immutable record exported from
`agentfootprint/context`. It defines `facts`, `limitations`, `scope`,
`evidenceRefs`, `nextSteps`, `domainDefinitions`, `objective` and
`completionRequirements`. `contextContractForModel()` formats those same
definitions as compact guidance for an existing `.system(...)` call or
`defineSteering({ id, prompt })`:
...
Facts remain sourced and scoped; limitations constrain claims. Absence does
not mean healthy, and no conflict does not establish completeness. References
need authorized resolution; proposed steps are neither executed nor authorized
by their presence. Domain definitions explain terms rather than observations.
Task goals and completion requirements do not establish completed results.

Nothing is injected automatically. This helper does not parse data, validate
answers, resolve evidence, select actions or provide prompt-injection protection.
Applications still own trusted context construction and enforcement. The mock
example `examples/context-engineering/23-context-contract.ts` checks instruction
and JSON delivery, not real-model reasoning quality.

`resolveEvidenceNeed(need, routes?, availableInputs?)`, on the same context
subpath, looks up exact application-declared evidence needs. It distinguishes an
unconfigured map, no matching route, and proposed routes with missing input
names. It does not call tools, verify access or prove that a source contains the
answer. Applications own domain mappings, scoped input validation and execution.
See the runnable `examples/context-engineering/23-context-contract.ts` example.

`EvidenceNeed` declares the missing evidence by an application-owned ID and description. `EvidenceRoute` declares a possible destination and its required input names. `EvidenceNeedResolution` distinguishes missing configuration, no matching declaration, and matched proposals with any missing inputs. These are metadata contracts, not evidence of access, availability or execution.
...
See dedicated guides: [Memory](doc:memory), [Skills](doc:skills), [Instructions](doc:instructions), [RAG](doc:rag), [Tools](doc:tools).
...
- [Quick Start](doc:quick-start) — first agent runs in 5 minutes
- [Memory guide](doc:memory) — defineMemory across 4 types × 7 strategies
- [Skills guide](doc:skills-explained) — context engineering for instructions
- [Reliability gate](doc:reliability-gate) — rules-based retry / fallback / fail-fast
- [Observability guide](doc:observability) — recorders, event taxonomy
