...
|---|---|
| Context engineering as a first-class runtime layer | [Why agentfootprint?](doc:why#the-new-class-of-bug) — Injection primitive |
| Every LLM call backtracks to *what / who / when / how* | [4 backtrack questions](doc:why#the-four-backtrack-questions) |
| Every code block in our docs is a real, runnable file in CI | The Phase 1 CodeFile + region marker infrastructure |
| Causal memory — the trace is a cache of the agent's thinking | [Causal deep dive](doc:causal-deep-dive) |
| Every named pattern is a recipe, not a class | [Patterns](doc:key-concepts#patterns--named-compositions) |
| Mocks-first dev → real-infra prod is a one-line swap per boundary | [Quick Start](doc:quick-start) |
| Multi-tenant isolation enforced at the storage boundary | [Memory guide](doc:memory) |
| Rules-based reliability gate around every LLM call | [Reliability gate](doc:reliability-gate) (v2.11.5+) |
...
- We are not the only framework with memory. Most have some flavor. Ours is the only one with **Causal** type (decision-evidence persistence) — see [Memory guide](doc:memory#the-four-types).
...
- We are not the only framework with reliability primitives. LangGraph's Pregel does retry; LangChain's `with_retry`/`with_fallbacks` exist. Ours adds **first-chunk arbitration** (streaming-aware) and routes via declarative rules — see [Reliability gate](doc:reliability-gate).
...
- [Quick Start](doc:quick-start) — first agent in 5 minutes
- [Why agentfootprint?](doc:why) — the bug-class argument with worked examples
- [Key Concepts](doc:key-concepts) — the 5-layer taxonomy
- [Causal memory deep dive](doc:causal-deep-dive) — the differentiator
