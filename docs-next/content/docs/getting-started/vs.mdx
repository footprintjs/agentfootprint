...
<Diagram name="triggers" alt="Every LLM call has 3 fixed slots (system, messages, tools); every flavor lands in one slot under one of 4 fixed triggers. The grid is the entire context-engineering surface — the layer every other framework leaves to the developer." />
