...
The ask got its typed half in 9.24.0 — [AskComponent](/docs/api/interfaces/AskComponent) names a registered
...
Deliberately shaped like the ask, for the same reasons:
  ...
  • `value` is small inline JSON, because it rides the resume call and lands
    in the run's history. Anything large belongs behind [from](/docs/api/interfaces/DecisionValue#from).
