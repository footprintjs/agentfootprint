...
| `agentEngineSessions` | `contract-shaped and tested` | Passes `sessionLifecycleConformance` against an injected double (`test/hosting/session-lifecycle-conformance.test.ts`). A field trial ([issue #2](https://github.com/footprintjs/agentfootprint/issues/2)) additionally exercised create, event-append, hydrate, corruption refusal, listing, pagination, awkward ids, and deletion live against the real service, all passing. | Declares **two** limitations, both reported by the battery by name. **(1) `ownership-fills-in-on-a-later-signed-turn`** — the service pins `userId` at create and refuses to change it, so a conversation that ran anonymously on turn one cannot be moved into somebody’s index on a later turn; its real owner IS recorded in the envelope, but the service-side index `ownerOf` and `listByUser` answer from cannot be moved. Field-confirmed independently by [issue #2](https://github.com/footprintjs/agentfootprint/issues/2). **(2) `list-by-user-is-newest-first`** — a listing is ordered by the service’s own immutable `update_time`, which this adapter also reports as `savedAt`, so a timestamp carried inside the envelope cannot influence the order (the two coincide whenever a session is written because it changed). Separately — and more seriously — **ownership conformance under concurrent/interleaved real-service writes is still NOT established.** A concurrent-ownership defect found by audit was fixed in 9.45.0 and is pinned by a deterministic regression (`test/hosting/agent-engine-ownership-race.test.ts`), but that is a double, and a double cannot establish either that the window exists on the real service or that closing it here closes it there. **This row does not move until a fresh live trial says so.** The same field trial found a security-sensitive ownership invariant that did not hold under repeated live interleavings; the finding is tracked privately (not a public issue) pending resolution. Until that is closed, this adapter should not be described as fully ownership-conformant, and the status here deliberately stays at the first rung rather than being promoted on the strength of the cases that did pass. |
