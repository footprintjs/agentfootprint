...
_Recorded 2026-07-28._

## In plain words

The package publishes **23 import paths** carrying **1028 distinct named exports**, plus **67 typed events**. For each one this report asks three separate questions: is it really *exported* (declared), is it *described in prose on the published docs site* (documented), and does a *real run actually use it* (exercised).

**210 of 1028 exports (20%) are described in prose on the site.** The rest split into five different problems, which is the whole point of keeping the columns apart:

- **40 exist, provably work, and are undocumented.** A reference run exercises them and no page on the site describes them. This is the honest headline number for "features that work and nobody has written about". It is the list to work through.
- **144 are already written up, just not published.** Prose about them exists inside the repo (`docs/`, `README.md`) but never made it onto the site. These are cheap wins: the writing is done, it needs moving.
...
- **615 are undocumented and no reference run touches them.** This report will not guess whether they work. They are reported as UNKNOWN, which is the honest answer, and they need a human pass.
- **123 are documented but no reference run exercises them.** For a function or a class that is the shape a dead or unimplemented feature has. For a type or an interface it is mostly noise, because a type is used, not called — so read that class by kind, and the tables below split it.

On events: **17** of the 67 typed events are both described on the site and were seen firing in a real run. **1** is described but was never observed firing — that is exactly the shape the resilience events had for months (fully declared, with payload types, and zero emitters), so this number is worth a look every time it moves. **49** are not described in prose on the site at all.
...
|---|---|
| every named export not in site prose | 818 |
| … of those, absent from every prose anywhere in the repo | 655 |
| only functions and classes (things you can call) | 206 |
| only exports on the root barrel | 156 |
| **functions and classes on the root barrel** | **28** |
| functions and classes that a reference run proves work | 15 |
| typed events | 49 |

The closest analogue to the remembered 36 is the **28 callable things on the root barrel with no prose description** — near enough that the old inventory was probably counting something like it, and far enough from 818 that quoting a single "undocumented" number without saying which rule produced it is how a figure like 36 drifts. Every table below states its rule.
...
These provably work — a reference run touches them — and no page on the site describes them. **40 symbols.**
...
| `AgentCoreLikeClient` | interface | `agentfootprint/memory-providers` |
| `BacktrackTrace` | interface | `agentfootprint/debug` `agentfootprint/observe` |
...
| `McpSdkClient` | interface | `agentfootprint/tool-providers` |
...
**Events described on the site but never observed firing (1).**

| Event | Described on |
|---|---|
| `agentfootprint.skill.activated` | `docs-next/content/docs/build/skills.mdx` |

**Functions and classes described on the site but not touched by any reference run (65).** The other 58 in this class are types, interfaces and constants, which a run cannot "call" — they are named in `docs/docs-truth/baseline.json` rather than here.
...
Prose about these exists in the repo (`docs/`, `README.md`) but nothing on the site mentions them. The writing is already done — this is a publishing job, not an authoring job, which makes it the cheapest class to close. **144 symbols.**
...
| `RunnerBase` | class | `agentfootprint` | `docs/MENTAL_MODEL.md`, `docs/design/local-observability-and-pii.md` |
...
| `Runner` | interface | `agentfootprint` | `README.md`, `docs/MENTAL_MODEL.md`, `docs/guides/concepts.md` |
...
| `contentHash` | function | `agentfootprint/debug` `agentfootprint/observe` | `docs/MENTAL_MODEL.md` |
...
| `toolChoiceRecorder` | function | `agentfootprint/observe` | `README.md`, `docs/guides/tool-catalog-lint.md` |
...
| `CredentialProvider` | interface | `agentfootprint/identity` | `docs/guides/agentcore.md` |
...
| `AblationSpec` | type | `agentfootprint/debug` `agentfootprint/observe` |
...
Nowhere in any prose, and no reference run covers them, so this report will not claim they work or that they are dead. Some are internal-shaped types that happen to be exported; some may genuinely be dead. **615 symbols.**
...
| … | | _495 more. Every undocumented name is listed in `docs/docs-truth/baseline.json`; for the full classified table run `node scripts/docs-truth-check.mjs --json out.json`_ |
...
|---|---|---|---|
| `agentfootprint` | 214 | 58 | 27% |
| `agentfootprint/cache` | 17 | 2 | 12% |
| `agentfootprint/events` | 24 | 4 | 17% |
| `agentfootprint/memory` | 140 | 24 | 17% |
| `agentfootprint/llm-providers` | 37 | 19 | 51% |
| `agentfootprint/memory-providers` | 11 | 3 | 27% |
| `agentfootprint/strategies` | 36 | 12 | 33% |
| `agentfootprint/observability-providers` | 28 | 6 | 21% |
| `agentfootprint/observe` | 351 | 35 | 10% |
| `agentfootprint/debug` | 215 | 24 | 11% |
...
| `agentfootprint/injection-engine` | 63 | 11 | 17% |
| `agentfootprint/tool-providers` | 16 | 8 | 50% |
| `agentfootprint/security` | 15 | 4 | 27% |
| `agentfootprint/identity` | 25 | 3 | 12% |
...
| `agentfootprint/embedders` | 6 | 0 | 0% |
...
- the 818 pre-existing undocumented exports recorded in the baseline → **pass**. They are known debt, not a surprise. Failing on them would turn the check red on day one and get it deleted in a week, which is worse than no check.
