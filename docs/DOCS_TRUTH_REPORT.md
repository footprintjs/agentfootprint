...
_Recorded 2026-08-13._

## In plain words

The package publishes **12 import paths** carrying **1544 distinct named exports**, plus **85 typed events**. For each one this report asks three separate questions: is it really *exported* (declared), is it *described in prose on the published docs site* (documented), and does a *real run actually use it* (exercised).

**759 of 1544 exports (49%) are described in prose on the site.** The rest split into five different problems, which is the whole point of keeping the columns apart:
...
- **123 are already written up, just not published.** Prose about them exists inside the repo (`docs/`, `README.md`) but never made it onto the site. These are cheap wins: the writing is done, it needs moving.
- **23 appear only inside a code sample** and nowhere in the surrounding text. A reader scanning the page never learns they exist, and site search does not find them.
- **606 are undocumented and no reference run touches them.** This report will not guess whether they work. They are reported as UNKNOWN, which is the honest answer, and they need a human pass.
- **659 are documented but no reference run exercises them.** For a function or a class that is the shape a dead or unimplemented feature has. For a type or an interface it is mostly noise, because a type is used, not called — so read that class by kind, and the tables below split it.

On events: **21** of the 85 typed events are both described on the site and were seen firing in a real run. **24** are described but were never observed firing — that is exactly the shape the resilience events had for months (fully declared, with payload types, and zero emitters), so this number is worth a look every time it moves. **40** are not described in prose on the site at all.
...
|---|---|
| every named export not in site prose | 785 |
| … of those, absent from every prose anywhere in the repo | 639 |
| only functions and classes (things you can call) | 191 |
...
The closest analogue to the remembered 36 is the **26 callable things on the root barrel with no prose description** — near enough that the old inventory was probably counting something like it, and far enough from 785 that quoting a single "undocumented" number without saying which rule produced it is how a figure like 36 drifts. Every table below states its rule.
...
|---|---|---|
| `docs-next/content/docs/**.mdx` (hand-written) | 88 | **Yes — the truth source.** This is what the published site renders and what a reader sees. |
...
| `rankEntries` | function | `agentfootprint/context` |
...
| `EntryScorer` | interface | `agentfootprint/context` |
...
**Events described on the site but never observed firing (24).**
...
| `agentfootprint.credential.failed` | `docs-next/content/docs/infrastructure/identity-and-credentials.mdx`, `docs-next/content/docs/infrastructure/on-premises.mdx` |
...
**Functions and classes described on the site but not touched by any reference run (231).** The other 428 in this class are types, interfaces and constants, which a run cannot "call" — they are named in `docs/docs-truth/baseline.json` rather than here.
...
| `checkSkillContracts` | function | `agentfootprint/context` |
...
| `formatCheckup` | function | `agentfootprint/context` |
...
| `pointerOf` | function | `agentfootprint/context` |
...
Prose about these exists in the repo (`docs/`, `README.md`) but nothing on the site mentions them. The writing is already done — this is a publishing job, not an authoring job, which makes it the cheapest class to close. **123 symbols.**
...
| `decideSkill` | function | `agentfootprint/context` | `docs/design/skill-graph-spec.md`, `docs/proposals/002-skill-graph.md` |
...
| `projectActiveInjection` | function | `agentfootprint/context` | `docs/MENTAL_MODEL.md` |
...
| `ActiveInjection` | interface | `agentfootprint/context` | `docs/MENTAL_MODEL.md` |
...
| `SkillRouting` | interface | `agentfootprint/context` | `docs/proposals/002-skill-graph.md` |
...
The name appears in a fenced block on the site and nowhere in the surrounding text. Reported as its own class rather than silently counted either way. **23 symbols.**
...
| `embeddingScorer` | function | `agentfootprint/context` |
| `keywordScorer` | function | `agentfootprint/context` |
...
Nowhere in any prose, and no reference run covers them, so this report will not claim they work or that they are dead. Some are internal-shaped types that happen to be exported; some may genuinely be dead. **606 symbols.**
...
| … | | _486 more. Every undocumented name is listed in `docs/docs-truth/baseline.json`; for the full classified table run `node scripts/docs-truth-check.mjs --json out.json`_ |
...
- the 785 pre-existing undocumented exports recorded in the baseline → **pass**. They are known debt, not a surprise. Failing on them would turn the check red on day one and get it deleted in a week, which is worse than no check.
