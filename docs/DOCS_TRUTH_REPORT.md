...
The package publishes **15 import paths** carrying **1997 distinct named exports**, plus **107 typed events**. For each one this report asks three separate questions: is it really *exported* (declared), is it *described in prose on the published docs site* (documented), and does a *real run actually use it* (exercised).

**1237 of 1997 exports (62%) are described in prose on the site.** The rest split into five different problems, which is the whole point of keeping the columns apart:
...
- **1032 are documented but no reference run exercises them.** For a function or a class that is the shape a dead or unimplemented feature has. For a type or an interface it is mostly noise, because a type is used, not called — so read that class by kind, and the tables below split it.
...
|---|---|
| every named export not in site prose | 760 |
| … of those, absent from every prose anywhere in the repo | 634 |
| only functions and classes (things you can call) | 176 |
...
**Functions and classes described on the site but not touched by any reference run (333).** The other 699 in this class are types, interfaces and constants, which a run cannot "call" — they are named in `docs/docs-truth/baseline.json` rather than here.
...
Prose about these exists in the repo (`docs/`, `README.md`) but nothing on the site mentions them. The writing is already done — this is a publishing job, not an authoring job, which makes it the cheapest class to close. **114 symbols.**
...
|---|---|---|---|
| `agentfootprint` | 518 | 370 | 71% |
| `agentfootprint/providers` | 140 | 106 | 76% |
...
| `agentfootprint/security` | 72 | 51 | 71% |
...
- the 760 pre-existing undocumented exports recorded in the baseline → **pass**. They are known debt, not a surprise. Failing on them would turn the check red on day one and get it deleted in a week, which is worse than no check.
