...
**84 typed events / 20 domains**, all `agentfootprint.*`: `composition.*`(8) `agent.*`(10)
`stream.*`(7) `context.*`(5 — the thesis) `memory.*`(5) `tools.*`(12) `validation.*`(1) `skill.*`(10) `permission.*`(4)
