...
**107 typed events / 24 domains**, all `agentfootprint.*`: `composition.*`(8) `agent.*`(13 — incl.
...
`checkin.*`(2) `middleware.*`(1) `embedding.*`(1) `risk.*`(1) `fallback.*`(1) `artifacts.*`(5 — the claim-check lifecycle) `map.*`(2 — the mount kernel's engagement standing: engaged / parked) `integrity.*`(2 — detected context errors filed as findings, plus the run-boundary disposition rows).
