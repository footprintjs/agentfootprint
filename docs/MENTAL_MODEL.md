...
NOTE: the current `buildDynamicAgentChart` (shipped, opt-in `reactStructure:'subflow'`) is the
LINEAR slot-chain interim — it gets the sf-llm-call boundary right (so Lens shows an LLM card) but
NOT yet the messageAPI merge-tree. This locked design supersedes it for the slot-internal shape.
...
- **`enable` namespace** — `.thinking()` `.logging()` (both deprecated v2.8) `.flowchart()`
  `.observability()` `.cost()` `.liveStatus()`. All route through the `EventDispatcher`, not the executor.
...
**Auto-attached recorders (Agent):** ContextRecorder, streamRecorder, agentRecorder, errorBridge,
evalRecorder, memoryRecorder, skillRecorder, toolsRecorder (always); costRecorder (if pricingTable);
permissionRecorder (if permissionChecker). LLMCall attaches a subset (no agent/tools/permission).
...
```
Client (stage)
  → sf-llm-call (SUBFLOW): Seed → sf-system-prompt → sf-messages → call-llm
...
The classic chart (still the default; the LLM is a flat `call-llm` STAGE):
...
Seed
  ...
  → Route(decider) → tool-calls (pausable) | final (subflow, propagateBreak:true)
  loopTo(sf-injection-engine)
```
...
### Dynamic ReAct chart (NEW — `src/core/agent/buildDynamicAgentChart.ts`)

A second builder (added 2026-05-30) that wraps the entire context-engineering + call region in an
...
(arrayMerge Replace). **Status: written + compiles, NOT yet wired into `Agent.ts`, NOT tested.**
Classic stays on `buildAgentChart`. (See §15 / git for resume steps.)
...
- **`buildDynamicAgentChart` not wired/tested** (§5) — the active in-flight work.
...
src/
  ...
  events/                  registry (EVENT_NAMES), payloads (54 shapes), dispatcher, types(EventMeta)
...
**Subpath exports:** `agentfootprint` (main), `/llm-providers`, `/memory-providers`,
`/observability-providers`, `/observe`, `/thinking`, `/security`, `/injection-engine`.
