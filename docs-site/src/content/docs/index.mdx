...
New questions cosine-match past queries; matching snapshots inject the stored past run; the LLM answers from what actually happened instead of re-deriving. See [Memory guide § Causal](/agentfootprint/guides/memory/#causal-memory--replay-decisions-not-just-messages).
...
- **65 typed events across 18 domains** — context, stream, agent, cost, permission, credential, eval, memory, skill, composition, tools, reliability, error, and more
