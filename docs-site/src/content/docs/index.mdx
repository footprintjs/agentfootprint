...
- **Per-iteration tool gating** — `gatedTools(inner, (name, ctx) => ctx.identity?.principal === 'admin')` exposes a different tool set per iteration based on caller identity and what the agent has learned
...
Per-iteration recomposition would burn tokens (everything re-builds every iteration) without caching. v2.6 ships a **provider-agnostic cache layer** — declare a `cache` policy (`'always'`, `'while-active'`, `'never'`, or a `{ until }` predicate) on a Skill / Steering / Instruction / Fact and the framework picks the right cache mechanism per provider:

| Provider | Mechanism | Honors the `cache` policy |
|---|---|---|
...
New questions cosine-match past queries; matching snapshots inject the stored past run; the LLM answers from what actually happened instead of re-deriving. *(Status: today the snapshot stores the query + final outcome; wiring the operator-level decision evidence into the snapshot is in progress — see the roadmap.)* See [Memory guide § Causal](/agentfootprint/guides/memory/#causal-memory--replay-decisions-not-just-messages).
...
| Cache strategy | `cache: 'never'` | `cache: 'always'` / `'while-active'` — provider strategy auto-selected (v2.6) |
...
- **63 typed events across 17 domains** — context, stream, agent, cost, permission, credential, eval, memory, skill, composition, tools, reliability, error, and more
