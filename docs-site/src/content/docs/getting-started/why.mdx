...
const entries = agent.getNarrativeEntries();
const sources = getGroundingSources(entries);  // tool results (truth)
const claims = getLLMClaims(entries);           // LLM output (to verify)

// sources: [{ orderId: 'ORD-1003', status: 'shipped', amount: 299 }]
// claims: ['Your order ORD-1003 is shipped. Total: $299.']
