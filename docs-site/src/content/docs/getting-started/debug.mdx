...
64 typed events across 18 domains. Your IDE auto-completes payload shapes per event name. See [Observability guide](/agentfootprint/guides/observability/) for the full taxonomy.
...
Every run persists a snapshot. Two weeks later, when the customer comes back asking why, the matching snapshot loads into the next prompt — the follow-up answers from the stored run, not a rerun. See [Memory guide § Causal](/agentfootprint/guides/memory/#causal-memory--replay-decisions-not-just-messages).
