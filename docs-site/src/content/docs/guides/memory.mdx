...
Causal memory is the differentiator. Other libraries' memory remembers what was said. agentfootprint's `defineMemory({ type: CAUSAL })` remembers the run itself, not just the messages. New questions cosine-match past queries; the matching stored run injects into the next prompt; the LLM answers from what actually happened rather than re-deriving. *(Status: today the snapshot stores the query + final outcome; wiring the operator-level decision evidence into the snapshot is in progress — every `decide()` condition, tool call, and token count lands in the snapshot when the evidence bridge ships.)*
...
For a deeper dive on how identity flows through the store + RAG indexing footgun, see [Memory store adapters](/agentfootprint/integrations/memory-stores/).
...
- [Auto memory (Hybrid)](/agentfootprint/guides/auto-memory/) — stack recent window + extracted facts + causal snapshots, each its own `.memory()` call
