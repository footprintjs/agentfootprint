...
## Tier-3 — `.enable.observability()` (v2.8) — grouped vendor strategies

For shipping events to a vendor backend (CloudWatch, Datadog, OTel, etc.), `.enable.observability(...)` accepts a typed strategy + optional `detach` driver. The strategy is the WHERE; the detach is the HOW (sync inline vs fire-and-forget):
...
|---|---|---|
| `agentcoreObservability` | ✅ v2.8.1 | `@aws-sdk/client-cloudwatch-logs` |
| `cloudwatchObservability` | v2.8.2 (next) | `@aws-sdk/client-cloudwatch-logs` |
| `xrayObservability` | v2.8.3 | `@aws-sdk/client-xray` |
| `otelObservability` | v2.9.x | `@opentelemetry/sdk-node` |
| `datadogObservability` | v2.9.x | `dd-trace` |
