...
Every API surface this essay describes — `defineSkill({ id, description, body, tools, surfaceMode, refreshPolicy })`, `resolveSurfaceMode(provider, model)`, and `SkillRegistry` — is shipped and runnable. The runtime polish for fully-divergent per-mode behavior (suppressing the system-prompt slot for `'tool-only'`, etc.) lands in v2.5; today's runtime treats every mode as the cross-provider-correct path the essay calls right. Consumers express intent today; behavior tightens silently.
</Aside>
