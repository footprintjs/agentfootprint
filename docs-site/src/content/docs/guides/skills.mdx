...
`SkillRegistry` methods: `register(skill)` · `replace(id, skill)` · `unregister(id)` · `get(id)` · `has(id)` · `list()` · `clear()` · `size` · `toTools()`. Throws on duplicate `register` (use `replace` for explicit overwrites). Throws on non-Skill flavor inputs.
