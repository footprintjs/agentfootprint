...
export function agentcoreObservability(opts: AgentcoreObservabilityOptions): ObservabilityStrategy {
  if (!opts.logGroupName) {
    throw new TypeError(
      `[agentcoreObservability] \`logGroupName\` is required. ` +
...
function createCloudWatchClient(region: string | undefined): CloudWatchLikeClient {
  let mod: CloudWatchSdkModule;
  try {
    mod = lazyRequire<CloudWatchSdkModule>('@aws-sdk/client-cloudwatch-logs');
  } catch {
    throw new Error(
      'agentcoreObservability requires the `@aws-sdk/client-cloudwatch-logs` peer dependency.\n' +
  ...
  if (!mod.CloudWatchLogsClient || !mod.PutLogEventsCommand) {
    throw new Error(
      'agentcoreObservability: `@aws-sdk/client-cloudwatch-logs` is installed but ' +
