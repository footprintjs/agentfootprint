...
## Declared datasets from any tool

`withDatasetArtifacts(tool, adapter)` wraps an existing tool without changing its
schema, source, permissions or argument contract. `adapter.describe(result, args)`
returns a plan with explicit dataset declarations and a `project(publications)`
function, or `undefined` to keep the result. It never infers that arbitrary arrays
are tables. A declaration has a unique `key`, an `artifact: ToolArtifactPutInput`,
and optional `source: ToolArtifactPutInput`. Schema, units, grain and coverage are
...
`{ status: 'stored', meta }` or `{ status: 'unavailable', reason }`. The existing
bound capability stamps origin and enforces scope and parent references. A source
ticket becomes an additional parent without replacing existing parents.

Writes are independent, not atomic. Source failure leaves its receipt unavailable
and does not prevent a row write; the projection must disclose that missing
source. A failed row write never yields a ticket. The helper rechecks successful
tickets after the batch because later writes may evict earlier ones. Read or
write errors yield `store-unavailable`; a missing ticket yields
`missing-or-expired`. A source orphaned by a row-write failure follows normal
retention. No rollback, pinning or post-publication availability is promised.

The wrapper skips the adapter entirely when `ctx.hasArtifacts` is false, preserving
the original result. A host that disallows inline data must retain its normal
projection/refusal policy. Declaration keys and input shapes, including an array
of strings for any declared `parentRefs`, are validated before the first write;
payload serialization, budgets and parent existence remain the
store's responsibility without an extra whole-payload scan. These helpers stage
materialized payloads, not remote handles or streaming datasets.

For MCP, opt into `resultMode: 'structured'` or the explicitly declared JSON-text
...
adapters. `datasetResult.ts` imports the Tool contract only as a type; it adds no
runtime dependency on Agent or the MCP SDK.
...
| `placement.ts`         | the placement threshold (Leg 3), PURE half: the `ArtifactPlacement` dial (`{ maxInlineChars }` on the object form `Agent.create({ artifacts: { store, placement } })` — placement cannot be SPELLED without a store), `placedResultKind(toolName, declared?)` = the ONE kind decision — the tool's declared `Tool.resultKind` when it has one (9.70.0), `tool-result/<toolName>` otherwise (the honest default vocabulary a consumer wants or presents), and `placedToolResult` — the ONE-shape substitute (`{placed: true, ref, kind, mediaType, bytes, reason}`, the TruncatedToolResult law). Precedence, stated once: tool `resultCeiling` FIRST (author's refusal), placement second (operator's ref-ing, judged on what the after-tool chain let through), `maxToolResultChars` truncation net LAST (then measures the ticket). The stored payload is the EXACT text the model would have read. A placed result is a ticket, not a refusal: effects are judged and steps advance. |

## Import direction

At runtime, `artifacts/` imports **only** `lib/` (lazyRequire, sqliteUnavailable) and
`memory/identity/` (the ONE scoping tuple — `ArtifactScope` _is_
`MemoryIdentity`, aliased, so two spellings of one isolation rule cannot
drift). `datasetResult.ts` also has erased type-only imports of `Tool` and
`ToolExecutionContext` from `core/`; these add no runtime dependency.
There are no runtime imports of `core/`, `events/`, or any recorder: the
...
- **`Tool.resultKind` is how a PLACED result becomes spendable** (9.70.0).
  The exact-match law above is what makes a ticket a promise — and it is also
  what made the framework's own default kind unspendable: placement mints
  `tool-result/<toolName>`, so a `wants: { dataset: 'dataset/rows' }`
  argument refused the very ticket placement had just minted, and consumers
  re-minted by hand at the seam. Declaring `resultKind: 'dataset/rows'` on
  the producing tool makes the MINT speak the consumer's vocabulary. The
  matcher is untouched: nothing here loosens `wants`. Omitted → the default
  kind, byte-identical. Resolved once, in `placedResultKind`.
