...
### Which charts mint a receipt (9.91.0)

Every chart in this library that hands a model a request mints one, and the one
exception is a chart that **cannot salt** its hashes rather than a chart nobody
got round to.

| chart | mints? | the salt |
|---|---|---|
| `Agent` (`dynamic`, `dynamic-grouped`) | yes | its own run id, per run |
| `LLMCall` | yes (9.91.0) | its own run id, per run — it owns its executor exactly as `Agent` does |
| `buildMessageApiChart` | when given one | `MessageApiChartDeps.getRunId` |
| `buildAgentMessageApiChart` | when given one | `AgentMessageApiChartDeps.getRunId` |

The two message-API charts are exported chart BUILDERS handed to an executor the
caller owns, and nothing in a stage's scope carries that executor's run id — so
the salt has to come from whoever starts the run:
...
Every hash a receipt carries is salted with the run id so a short system prompt
or a two-word turn cannot be fingerprinted across runs (law 3 below), and a
receipt is committed state that travels inside recordings; a chart that minted
with an empty salt would ship dictionary-attackable fingerprints by default.
The view is rebuilt exactly as before and says what it has not got:
`no-receipt-on-chart`, cause `'no-receipt-committed'`.

`Agent` and `LLMCall` both take `recordReceipt: false` to decline the mint on a
run that does not want to pay for it — one commit-log value and a SHA-256 per
piece, per message and per tool schema.

### Measuring the initial prepared request

`receipt.requestMeasurement` records the size of the complete prepared canonical
request, including the final tool schemas. The shared `buildReceipt` mint adds
it whenever a receipt is enabled. The field is optional for readers because
older recordings lack it; absence means unavailable measurement, never zero.
It records sizes only, without copying request content into the receipt.

Both result shapes declare `boundary: 'initial-prepared-request'` and
`format: 'json-utf8-v1'`:

- `status: 'measured'` carries `total: { jsonChars, jsonBytes }` and `slots` with
  optional `systemPrompt`, `messages` and `tools` sizes in the same shape.
- `status: 'unavailable'` carries
  `reason: 'unsupported-value' | 'cyclic-value' | 'measurement-limit'`.
  An unavailable measurement does not fail the provider call.

Internal safety limits cap measurement at 100,000 visited values, 64 nested
containers, and 4,000,000 string/key UTF-16 code units while copying or JSON
characters when serialized; exceeding a limit reports `measurement-limit`
without truncating or rejecting the provider request. These fixed limits bound
telemetry work and require no caller configuration.

`jsonChars` counts UTF-16 code units in the JSON serialization; `jsonBytes`
counts its UTF-8 bytes. The complete request excludes only its root `signal`
field. Each present slot is serialized separately, including JSON quoting and
structure, so slot sizes do **not** sum to `total`: the whole request also has
field names, delimiters and other request fields. These are neither raw text
lengths nor token counts. `RequestMeasurement` and `RequestJsonSize` are public
types from `agentfootprint`; measurement adds no new runtime export.

```ts
import { receiptAt } from 'agentfootprint';

const measurement = receiptAt(agent.getSnapshot()!, 1)?.requestMeasurement;
if (measurement?.status === 'measured') {
  console.log(measurement.total.jsonBytes);
  console.log(measurement.slots.tools?.jsonBytes);
} else {
  console.log(measurement?.reason ?? 'Not recorded');
}
```

The boundary is after initial request preparation, including cache preparation
where configured. It does not measure later reliability retries, provider
decorators, vendor serialization or HTTP transport. Existing slot budgets,
advisory pressure events and their counters remain unchanged; this measurement
does not enforce a limit or change what the provider receives.
...
The receipt-only `requestMeasurement` field is read through `receiptAt`, not
recomputed from `servedAt`. A `measured` status supplies JSON character and
UTF-8 byte counts for the initial prepared request; `unavailable` supplies its
reason without sizes. An older receipt with no field supplies no measurement.
None of these states implies zero, token usage, HTTP wire size or retry totals.
The `no-receipt-on-chart` gap covers losing the whole receipt; it is not raised
merely because this optional field is absent on an older receipt.
...
provider PORT — from the agent charts' `stages/callLLM.ts`, from `LLMCall.ts` ·
`callLLM`, and from the two message-API charts through `messageApiReceipt.ts`. Three things sit downstream of that
