 ...
 * stops at 64 KB and every later leaf is `too-large`.
 */
...
const ANY_EVENT = ['#meta/runId', '#meta/sessionId'];
...
/** Is this pointer's leaf one the allow-list admits (and the deny list does not refuse)? */
export function isShowable(p: RecordPointer): boolean {
  switch (p.kind) {
    case 'event': {
      ...
      return (SHOW_ME_ALLOW_LIST[type] ?? []).includes(shape(p.path));
    }
    case 'state':
...
export function showLeaves(
  ...
  for (const p of accountPointers(account)) {
    ...
    const size = key.length + JSON.stringify(out).length;
    if (bytes + size > MAX_SHOWN_BYTES) out = { withheld: 'too-large' };
    bytes += key.length + JSON.stringify(out).length;
    shown[key] = out;
  }
  return shown;
}
