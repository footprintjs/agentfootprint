...
function buildReport(result, surface, events, exercised, docs, baseline) {
  ...
  L.push('');
  // The BASELINE's date, never today's: the report is a generated file that CI
  // regenerates and diffs, so a wall-clock date made every PR opened on a new
  // day fail `generated` with nothing actually stale. The date moves only when
  // the baseline is re-recorded (`npm run docs:truth:baseline`).
  L.push(`_Recorded ${baseline?.recordedAt ?? new Date().toISOString().slice(0, 10)}._`);
  L.push('');
