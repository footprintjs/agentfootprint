  ...
  describe(result) {
    return {
      ...
      project(publications) {
        ...
        return {
          dataset: {
            ref: publication.artifact.meta.ref,
            kind: publication.artifact.meta.kind,
            rows: result.rows.length,
            sourceField: publication.key,
          },
          sourceEvidence:
            publication.source?.status === 'stored'
...
function consumer(seen: { rows?: unknown; meta?: ArtifactMeta; calls: number }) {
  return defineTool<{ dataset: string }, string>({
    name: 'sum_rows',
    description: 'Sum the complete stored dataset; pass its reference.',
    inputSchema: {
      type: 'object',
...
async function source(mode: 'local' | 'http-backed' | 'mcp') {
  ...
  if (mode === 'mcp') {
    const connection = {
      listTools: async () => ({
        tools: [{ name: 'lookup_rows', description: 'Synthetic ledger lookup', inputSchema }],
      }),
      callTool: vi.fn(async (request: { name: string; arguments?: Record<string, unknown> }) => {
        ...
        return {
          content: [{ type: 'text', text: 'PROSE_MIRROR_NOT_THE_DATASET' }],
          structuredContent: await collected(),
        };
      }),
      close: async () => {},
    };
  ...
  const fetchRows = vi.fn(async (url: string, options: { signal?: AbortSignal }) => {
    ...
    return new Response(JSON.stringify(await collected()), {
  ...
  const tool: Tool<Args, typeof raw> = {
    ...
    async execute(args, ctx) {
      ...
      return mode === 'http-backed'
        ? await (
            await fetchRows(`https://synthetic.invalid/accounts/${args.account}`, {
              signal: ctx.signal,
            })
          ).json()
        : await collected();
...
describe('dataset adapter at native Agent boundaries', () => {
  it.each(['local', 'http-backed', 'mcp'] as const)(
    '%s lookup publishes scoped references and a follow-up consumes complete rows without collecting again',
    async (mode) => {
        ...
        provider: mock({
          respond(request) {
            ...
            if (requests.length === 3)
              return call('sum_rows', { dataset: ticket!.dataset.ref }, 'sum-call');
      ...
      try {
        expect(await agent.run({ message: 'Load demo rows.', identity: scope })).toBe(
          'Dataset ready.',
        );
        expect(ticket?.dataset).toMatchObject({
          kind: 'dataset/rows',
          rows: 3,
          sourceField: 'rows',
        });
        ...
        expect(saved?.meta.origin).toMatchObject({
  ...
  it.each(['tenant', 'principal', 'conversationId', 'deleted', 'expired', 'source-kind'] as const)(
    '%s boundary refuses a published ticket before consumer execution',
    async (boundary) => {
      let now = 1_000;
      ...
      const producer = Agent.create({
        ...
        .tool(withDatasetArtifacts(fixture.tool, adapter))
        .build();
      await producer.run({ message: 'Load demo rows.', identity: scope });
      expect(ticket?.dataset).toEqual(
        expect.objectContaining({ ref: expect.stringMatching(/^art_/) }),
      );
      ...
      const otherScope = ['tenant', 'principal', 'conversationId'].includes(boundary)
        ? { ...scope, [boundary]: 'other' }
        : scope;
      const seen = { calls: 0 };
      const requests: string[] = [];
      const refused: unknown[] = [];
      const reader = Agent.create({
        artifacts: store,
        model: 'mock',
        provider: mock({
          respond(request) {
            ...
            return requests.length === 1
              ? call('sum_rows', { dataset: ref }, 'refused-call')
              : 'Dataset unavailable.';
          },
        }),
      })
      ...
      expect(
        await reader.run({ message: 'Total the earlier dataset.', identity: otherScope }),
      ).toBe('Dataset unavailable.');
      expect(seen.calls).toBe(0);
      expect(fixture.collected).toHaveBeenCalledTimes(1);
      expect(refused).toEqual(
        expect.arrayContaining([expect.objectContaining({ op: 'dispatch', ref })]),
      );
  ...
  it('keeps a storeless native tool result and never invokes the projection', async () => {
    ...
    const tool = defineTool<Args, typeof raw>({
      name: 'lookup_rows',
    ...
    const agent = Agent.create({
      ...
      provider: mock({
        respond(request) {
          if (++calls === 1) return call('lookup_rows', { account: 'demo' });
          received = JSON.parse(lastTool(request));
