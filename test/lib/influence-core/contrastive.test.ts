...
const ev = (id: string, text: string): { id: string; text: string; ancestorTexts: string[] } => ({ id, text, ancestorTexts: [] });
const evidence = [
...
describe('scoreContrastiveInfluence — the confound fix', () => {
  it('plain influence is fooled by the topical innocent; contrastive is not', async () => {
    ...
    const contrast = await scoreContrastiveInfluence({
      evidence, answerText: 'ANSWER_DENY', referenceText: 'REFERENCE_APPROVE', embedder,
    });
  ...
  it('returns InfluenceScore[] sorted descending', async () => {
    const r = await scoreContrastiveInfluence({ evidence, answerText: 'ANSWER_DENY', referenceText: 'REFERENCE_APPROVE', embedder });
...
describe('scoreContrastiveInfluence — integration', () => {
  ...
  it('composes with rankingConfidence (same InfluenceScore[] shape)', async () => {
    const scores = await scoreContrastiveInfluence({ evidence, answerText: 'ANSWER_DENY', referenceText: 'REFERENCE_APPROVE', embedder });
...
describe('scoreContrastiveInfluence — property', () => {
  it('answer == reference ⇒ all contrastive FA are ~0 (no signal when there is no contrast)', async () => {
    const r = await scoreContrastiveInfluence({ evidence, answerText: 'ANSWER_DENY', referenceText: 'ANSWER_DENY', embedder });
  ...
  it('preserves all evidence ids exactly once for arbitrary small inputs', async () => {
    for (let n = 1; n <= 4; n++) {
      const evs = Array.from({ length: n }, (_, i) => ev(`s${i}`, i % 2 ? 'culprit (caused the deny)' : 'filler (unrelated)'));
      // unique ids required; texts may repeat
      const r = await scoreContrastiveInfluence({ evidence: evs, answerText: 'ANSWER_DENY', referenceText: 'REFERENCE_APPROVE', embedder });
...
describe('scoreContrastiveInfluence — security & robustness', () => {
  it('duplicate evidence ids fail loud (same contract as scoreInfluence)', async () => {
    await expect(
      scoreContrastiveInfluence({
        ...
        answerText: 'ANSWER_DENY', referenceText: 'REFERENCE_APPROVE', embedder,
      }),
  ...
  it('malformed weights fail loud, attributed to scoreContrastiveInfluence (same contract as scoreInfluence)', async () => {
    ...
    ).rejects.toThrow(/scoreContrastiveInfluence: weight 'fa' must be a finite non-negative number/);
    await expect(
      scoreContrastiveInfluence({
        evidence, answerText: 'ANSWER_DENY', referenceText: 'REFERENCE_APPROVE', embedder,
...
describe('scoreContrastiveInfluence — performance & load', () => {
  it('200 evidence items score promptly (fake embedder isolates the math)', async () => {
    const big = Array.from({ length: 200 }, (_, i) => ev(`s${i}`, i % 2 ? 'culprit (caused the deny)' : 'policy (innocent, topical)'));
    const t0 = performance.now();
    const r = await scoreContrastiveInfluence({ evidence: big, answerText: 'ANSWER_DENY', referenceText: 'REFERENCE_APPROVE', embedder });
