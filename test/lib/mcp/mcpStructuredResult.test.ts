...
const declaration = {
  name: 'inventory',
  description: 'Declared inventory',
...
function connection(result: unknown): McpConnection {
  return {
    listTools: async () => ({ tools: [declaration] }),
    callTool: vi.fn(async () => result as McpCallToolResult),
    close: async () => {},
  };
}
...
describe('MCP declared structured results', () => {
  it('retains the structured object instead of its prose mirror, with declaration and source unchanged', async () => {
    const data = JSON.parse(
      '{"rows":[{"latency":0,"missing":null,"ok":false}],"__proto__":{"own":true}}',
    );
    const t = await tool(
      { ...text('A prose mirror, not the dataset.'), structuredContent: data },
      'structured',
    );
  ...
  it('uses the same mode through the library-built connection and after refresh', async () => {
    const sdk: McpSdkClient = {
      ...connection({ ...text('mirror'), structuredContent: { rows: [] } }),
      connect: vi.fn(async () => {}),
    };
    const client = await mcpClient({
      transport: { transport: 'stdio', command: 'unused' },
      _client: sdk,
  ...
  it('lets an ordinary execution wrapper store exact structured rows under the real invocation scope', async () => {
    ...
    const imported = await tool(
      { ...text('unused mirror'), structuredContent: data },
      'structured',
    );
      ...
      provider: mock({
        respond: (request) => {
          ...
          return requests.length === 1
            ? { toolCalls: [{ id: 'structured-call', name: 'inventory', args: {} }] }
            : 'Stored result ready.';
        },
      }),
    })
    ...
    expect(record?.meta.origin).toMatchObject({
  ...
  it.each([undefined, 'text'] as const)(
    'keeps default/explicit text and legacy behavior (%s)',
    async (mode) => {
      expect(
        await (
          await tool({ ...text('mirror'), structuredContent: { rows: [1] } }, mode)
        ).execute({}),
      ).toBe('mirror');
      expect(await (await tool({ toolResult: { rows: [] }, content: [] }, mode)).execute({})).toBe(
        '{"rows":[]}',
      );
      expect(
        await (
  ...
  it('parses one JSON object text block only when fallback was explicitly enabled', async () => {
    ...
    expect(await (await tool(result, 'structured-or-json')).execute({})).toEqual({
      rows: [],
      unknown: null,
      zero: 0,
      enabled: false,
    });
    await expect((await tool(result, 'structured')).execute({})).rejects.toThrow(
      /structuredContent/,
    );
  ...
  it('prefers present structured content over the explicit JSON fallback', async () => {
    expect(
      await (
        await tool(
          { ...text('{"rows":[1]}'), structuredContent: { rows: [] } },
          'structured-or-json',
        )
      ).execute({}),
    ).toEqual({ rows: [] });
  ...
  it.each([null, [], 0, 'secret-payload', { invalid: undefined }, { invalid: Number.NaN }])(
    'refuses malformed structured content without leaking or falling back (%j)',
    async (structuredContent) => {
      const t = await tool(
        { ...text('{"fallback":"secret-payload"}'), structuredContent },
        'structured-or-json',
      );
  ...
  it('refuses missing structured data and cyclic/non-JSON values', async () => {
    await expect(
      (await tool(text('No structured content'), 'structured')).execute({}),
    ).rejects.toThrow(/structuredContent/);
    const cyclic: Record<string, unknown> = {};
    cyclic.self = cyclic;
    for (const data of [cyclic, { value: 1n }, { value: new Date() }, { value: () => 1 }]) {
      await expect(
        (await tool({ content: [], structuredContent: data }, 'structured')).execute({}),
      ).rejects.toThrow(/structuredContent/);
    }
  });
  ...
  it('does not execute accessor fields at the structured-result boundary', async () => {
    for (const key of ['structuredContent', 'content', 'isError']) {
      const get = vi.fn(() => {
        throw new Error('SENSITIVE_GETTER');
      });
      const result = Object.defineProperty({}, key, { enumerable: true, get });
      await expect((await tool(result, 'structured-or-json')).execute({})).rejects.toThrow(
        /structuredContent/,
      );
  ...
  it('enforces documented decoding bounds without truncating a accepted object', async () => {
    ...
    expect(
      await (await tool({ content: [], structuredContent: data }, 'structured')).execute({}),
    ).toEqual(data);
    await expect(
      (await tool({ content: [], structuredContent: { next: data } }, 'structured')).execute({}),
    ).rejects.toThrow(/over-limit/);
    await expect(
      (
        await tool(
          { content: [], structuredContent: { values: new Array(1_000_000).fill(null) } },
          'structured',
        )
      ).execute({}),
    ).rejects.toThrow(/over-limit/);
    await expect(
      (await tool(text(' '.repeat(16_000_000) + '{}'), 'structured-or-json')).execute({}),
    ).rejects.toThrow(/over-limit/);
  });

  it('allows repeated JSON subobjects but refuses sparse arrays and extra non-JSON properties', async () => {
    const child = { value: 0 },
      record = { first: child, second: child };
    expect(
      await (await tool({ content: [], structuredContent: record }, 'structured')).execute({}),
    ).toEqual(record);
  ...
  it.each([
    ...
    'never treats prose, scalar/array, mixed blocks or legacy output as declared structured data (%j)',
    async (result) => {
      await expect((await tool(result, 'structured-or-json')).execute({})).rejects.toThrow(
        /structured|JSON/,
      );
  ...
  it.each(['structured', 'structured-or-json'] as const)(
    'honors isError before any structured decoding (%s)',
    async (mode) => {
      for (const result of [
        { ...text('backend unavailable'), structuredContent: { rows: [] }, isError: true },
  ...
  it.each(['structured', 'structured-or-json'] as const)(
    'preserves tool-authored correction text without reading structured error data (%s)',
    async (mode) => {
      const readStructured = vi.fn(() => {
        throw new Error('PRIVATE_STRUCTURED_ERROR_ROWS');
      });
      const result = {
        isError: true,
        content: [
      ...
      expect((error as Error).message).toContain(
        'HTTP 422: node must be complete\nUse the full node identifier.',
      );
  ...
  it.each(['structured', 'structured-or-json'] as const)(
    'bounds complete error diagnostics without truncating them into advice (%s)',
    async (mode) => {
      const atLimit = 'x'.repeat(8_192);
      await expect(
        (await tool({ ...text(atLimit), isError: true }, mode)).execute({}),
      ).rejects.toThrow(atLimit);
      for (const content of [
        text(atLimit + 'x').content,
        [
          { type: 'text', text: atLimit },
          { type: 'text', text: 'extra' },
        ],
        Array.from({ length: 33 }, () => ({ type: 'text', text: 'tool advice' })),
        [],
        [{ type: 'text', text: '' }],
        [{ type: 'image', text: 'not declared text' }],
          ...
          await tool(
            {
              content,
              isError: true,
              structuredContent: { secret: 'PRIVATE_STRUCTURED_ERROR_ROWS' },
            },
            mode,
          )
        )
        ...
        expect(error).toBeInstanceOf(Error);
        expect((error as Error).message).toContain(
          'returned an error; its structured result was not read.',
        );
  ...
  it.each(['structured', 'structured-or-json'] as const)(
    'reads only own data properties for error diagnostics (%s)',
    async (mode) => {
      const unsafe = vi.fn(() => {
  ...
  it('refuses an unknown mode before calling a connection', async () => {
    ...
    await expect(mcpClient({ connection: c, resultMode: 'guess' as Mode })).rejects.toThrow(
      /resultMode/,
    );
  ...
  it('keeps the curated mock usable for structured and explicitly JSON-text producers', async () => {
    const structured = mockMcpClient({
      resultMode: 'structured',
      tools: [
        { ...declaration, handler: async () => ({ content: [], structuredContent: { rows: [] } }) },
      ],
    });
    expect(await (await structured.tools())[0]!.execute({})).toEqual({ rows: [] });
    const json = mockMcpClient({
      resultMode: 'structured-or-json',
      tools: [{ ...declaration, handler: async () => '{"rows":[]}' }],
    });
