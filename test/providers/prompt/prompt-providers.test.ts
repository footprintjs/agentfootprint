...
describe('staticPrompt', () => {
  it('returns the same string every time', () => {
    ...
    expect(provider.resolve(baseCtx).value).toBe('You are helpful.');
    expect(provider.resolve({ ...baseCtx, turnNumber: 5 }).value).toBe('You are helpful.');
  });

  it('handles empty string', () => {
    expect(staticPrompt('').resolve(baseCtx).value).toBe('');
  });

  it('handles multi-line prompts', () => {
    const prompt = 'Line 1.\nLine 2.\nLine 3.';
    expect(staticPrompt(prompt).resolve(baseCtx).value).toBe(prompt);
  });
});
...
describe('templatePrompt', () => {
  it('interpolates explicit variables', () => {
    ...
    expect(provider.resolve(baseCtx).value).toBe('You are a code reviewer.');
  });
