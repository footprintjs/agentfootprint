...
| 13 | [`13-live-state.ts`](features/13-live-state.ts) | Live state — O(1)  | liveStateRecorder() bundles three trackers (LLM / tool / turn) on the BoundaryStateTracker storage primitive. Subscribe once, read O(1) at any moment. |
