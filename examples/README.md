...
| 06 | [`06-detached-observability.ts`](features/06-detached-observability.ts) | Detached observability — non-blocking telemetry export | Wire the new `detach` option on `enable.observability` so slow exporters never block the agent loop. Drain via `flushAllDetached` on shutdown. |
