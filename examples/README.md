...
| 54 | [`54-skill-graph-front-door.ts`](features/54-skill-graph-front-door.ts) | Skill graph — the front door: rules as data, scoped flat tools, deferred body checks | Start rules declared as data (match: RegExp \| { keywords }) so the check-up can compare them (overlapping-rules, rules-shadowed-by-order) and toMermaid can caption them; rule-id-exists refuses a rule naming an unknown skill; scopeTools reaches the FLAT arm (autoActivate on every wired skill, default false until 10.0.0); and body-contract checks defer to Agent build, where the full tool registry finally exists. |
